...
static int setup_max_cpus_proc_write(struct file *f, const char *data, size_t len, loff_t *offset)
{
	int r;
	unsigned int cpus_val;

	r = kstrtouint_from_user(data, len, 0, &cpus_val);

	if (r)
		return -EINVAL;

	setup_max_cpus = cpus_val;

	return len;
}

static int setup_max_cpus_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", setup_max_cpus);
...
#ifdef CONFIG_MACH_LGE
	const struct pentry entries_max_cpus[] = {
		PROC_ENTRY(setup_max_cpus),
	};
#endif
...
#ifdef CONFIG_MACH_LGE
	for (i = 0; i < ARRAY_SIZE(entries_max_cpus); i++) {
		if (!proc_create(entries_max_cpus[i].name, S_IRUGO | S_IWUSR | S_IWGRP, dir, entries_max_cpus[i].fops))
			return -ENOMEM;
	}
#endif
