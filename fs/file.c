...
#ifdef CONFIG_LGE_ETRS
			if (lentry->name != NULL)
				lge_ramconsole_log("[FDLEAK]OverAllocFDError(PID:%d fileName:%s Num:%d)\n",
						pid, lentry->name, num_of_entry);
			else
				lge_ramconsole_log("[FDLEAK]OverAllocFDError(PID:%d fileName:%s Num:%d)\n",
						pid, "NULL", num_of_entry);
...
#ifdef CONFIG_LGE_ETRS
	if (sum_fds_of_pid)
		lge_ramconsole_log("[FDLEAK]OverAllocFDError(PID:%d totalFDs:%d)\n", pid, sum_fds_of_pid);
#endif
...
#ifdef CONFIG_LGE_ETRS
		lge_ramconsole_log("[FDLEAK][%d:%s]fd over RLIMIT_NOFILE:%ld\n",
			current->pid, current->comm, rlimit(RLIMIT_NOFILE));
#endif
