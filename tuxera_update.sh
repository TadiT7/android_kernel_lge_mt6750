...
build_package() {
    ...
    echo "$archdirs" | grep arm64 > /dev/null && (
        arm64_need_arm_files="arch/arm/include/asm/opcodes.h arch/arm/include/asm/xen/hypervisor.h"
...
if ! options=$(getopt -o pahuv -l target:,user:,pass:,use-package:,source-dir:,output-dir:,version:,cache-dir:,server:,extraargs:,max-cache-entries:,admin:,upgrade,help,ignore-cert,no-check-certificate,use-curl,use-wget,no-excludes,use-cache,verbose,latest,retry-on-connection-fail,max_build_time_sec: -- "$@")
then
    usage
fi
...
do
    ...
    --admin) admin="$2" ; shift;;
    --retry-on-connection-fail) retry_on_conn_fail="yes" ; shift;;
    --max_build_time_sec) max_build_time_sec="$2" ;;
    (--) shift; break;;
