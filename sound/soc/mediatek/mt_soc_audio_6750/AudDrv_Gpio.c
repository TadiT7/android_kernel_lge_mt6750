...
enum audio_system_gpio_type {
	...
	GPIO_HPANASWITCH_EN_HIGH,
	GPIO_HPANASWITCH_EN_LOW,
	GPIO_EXTAMP2_HIGH,
	GPIO_EXTAMP2_LOW,
	GPIO_RCVSPK_HIGH,
	GPIO_RCVSPK_LOW,
...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
	...
	[GPIO_HPANASWITCH_EN_HIGH] = {"hpanaswitch-en-pullhigh", false, NULL},
	[GPIO_HPANASWITCH_EN_LOW] = {"hpanaswitch-en-pulllow", false, NULL},
...
int AudDrv_GPIO_HP_ANA_Select(int bEnable)
{
	...
	if (bEnable == 1) {
		if (aud_gpios[GPIO_HPANASWITCH_EN_HIGH].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_HPANASWITCH_EN_HIGH].gpioctrl);
			if (retval)
		...
		if (aud_gpios[GPIO_HPANASWITCH_EN_LOW].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_HPANASWITCH_EN_LOW].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_HPANASWITCH_LOW] pins\n");
		}
	}
...
int AudDrv_GPIO_EXTAMP_Select_With_Mode(int bEnable, int mode)
{
	int retval = 0;
	int i = 0;
