...
{
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_HP_ANA_SWITCH];
	pr_debug("%s() switch status = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	return 0;
}
...
#else
#ifdef CONFIG_LGE_SND_SOC_AW87318
		AudDrv_GPIO_EXTAMP_Select_With_Mode(false, 3);
		pr_debug("AW87318 Off\n");
#else
...
#else
#ifdef CONFIG_LGE_SND_SOC_AW87318
		AudDrv_GPIO_EXTAMP_Select_With_Mode(true, 3);
		pr_debug("AW87318 On\n");
#else
...
static bool GetAdcStatus(void)
{
	...
	for (i = AUDIO_ANALOG_DEVICE_IN_ADC1; i < AUDIO_ANALOG_DEVICE_MAX; i++) {
		if ((mCodec_data->mAudio_Ana_DevicePower[i] == true)
		    ...
		    && (i != AUDIO_ANALOG_DEVICE_HP_ANA_SWITCH))
#else
		    && (i != AUDIO_ANALOG_DEVICE_RECEIVER_SPEAKER_SWITCH))
#endif
			return true;
