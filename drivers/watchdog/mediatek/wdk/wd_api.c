...
//[LGE_UPDATE_E] DMS_SYSTEM dms-fota@lge.com
		...
		wd_api->wd_dram_reserved_mode(false);
		lge_set_reboot_reason(LGE_REBOOT_REASON_LAF_RESTART);
	} else if (cmd && !strncmp(cmd, "dm-verity device corrupted", 26)){
		lge_set_reboot_reason(LGE_REBOOT_REASON_DMVERITY_CORRUPTED);
	} else if (cmd && !strncmp(cmd, "wallpaper_fail", 14)){
		lge_set_reboot_reason(LGE_REBOOT_REASON_WALLPAPER_CORRUPTED);
	} else {
		reboot = 1;
	}
