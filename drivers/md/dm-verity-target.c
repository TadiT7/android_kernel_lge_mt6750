...
#ifdef CONFIG_DM_VERITY_CORRUPT_TEST
#define MAX_COUNT_CORRUPT_TEST (5)
static signed dm_verity_force_corrupt_metadata_block = 0; //metadata
static signed dm_verity_force_corrupt_data_block = 0; //data
module_param_named(corrupt_metadata_block, dm_verity_force_corrupt_metadata_block, uint, S_IRUGO | S_IWUSR);
module_param_named(corrupt_data_block, dm_verity_force_corrupt_data_block, uint, S_IRUGO | S_IWUSR);
#endif // CONFIG_DM_VERITY_CORRUPT_TEST
...
#ifdef CONFIG_DM_VERITY_CORRUPT_TEST
		if (dm_verity_force_corrupt_metadata_block > 0) {
			int i;
			if (dm_verity_force_corrupt_metadata_block > MAX_COUNT_CORRUPT_TEST)
				dm_verity_force_corrupt_metadata_block = MAX_COUNT_CORRUPT_TEST;

			for (i = 0; i < (1 << v->hash_dev_block_bits); i++)
				data[i] ^= 0xFF;
			DMERR("dm-verity force corrupt metadata block!!! [%d]", dm_verity_force_corrupt_metadata_block);
			dm_verity_force_corrupt_metadata_block--;
		}
#endif // CONFIG_DM_VERITY_CORRUPT_TEST
...
{
#ifdef CONFIG_DM_VERITY_CORRUPT_TEST
	if (dm_verity_force_corrupt_data_block > 0) {
		int i;
		if (dm_verity_force_corrupt_data_block > MAX_COUNT_CORRUPT_TEST)
			dm_verity_force_corrupt_data_block = MAX_COUNT_CORRUPT_TEST;

		for (i = 0; i < len; i++)
			data[i] ^= 0xFF;
		DMERR("dm-verity force corrupt data block!!! [%d]", dm_verity_force_corrupt_data_block);
		dm_verity_force_corrupt_data_block--;
	}
#endif // CONFIG_DM_VERITY_CORRUPT_TEST
