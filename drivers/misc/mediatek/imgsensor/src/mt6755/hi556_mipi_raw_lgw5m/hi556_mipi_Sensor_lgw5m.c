...
#include "hi556_mipi_Sensor_lgw5m.h"

#define PFX "hi556_camera_sensor_lgw5m"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define Hi556_LGW5M_OTP_FUNCTION 0

#if Hi556_LGW5M_OTP_FUNCTION //Hi-556 OPT
void HI556_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 g_gain,kal_uint32 b_gain);
kal_uint16 HI556_Lgw5m_Sensor_OTP_read(kal_uint16 otp_addr);
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI556_SENSOR_ID_LGW5M,
...
#if 1
static kal_uint16 read_w5m_moduleid(kal_uint32 addr)
{
...
void HI556OTPLGW5mSetting(void)
{
	LOG_INF("%s enter\n",__func__);
...
#if Hi556_LGW5M_OTP_FUNCTION //Hi-556 OPT fuction begin
...
void HI556_Sensor_OTP_info(void)
{
	...
	for(index = 0;index < 3;index++)
	{
		...
		if(0x55 == HI556_Lgw5m_Sensor_OTP_read(start_address))
		{
			for(i=0;i<7;i++)
			{
				infocheck += HI556_Lgw5m_Sensor_OTP_read(start_address+i);
			}
			ModuleID = HI556_Lgw5m_Sensor_OTP_read(start_address+1);
			LensID = HI556_Lgw5m_Sensor_OTP_read(start_address+2);
			VCMID = HI556_Lgw5m_Sensor_OTP_read(start_address+3);
			Year = HI556_Lgw5m_Sensor_OTP_read(start_address+4);
			Month = HI556_Lgw5m_Sensor_OTP_read(start_address+5);
			Day = HI556_Lgw5m_Sensor_OTP_read(start_address+6);
			checksum = HI556_Lgw5m_Sensor_OTP_read(start_address+7);
		}
...
void HI556_Sensor_calc_wbdata(void)
{
	...
	for(index = 0;index < 3;index++)
	{
		...
		if(0x55 == HI556_Lgw5m_Sensor_OTP_read(start_address))
		{
			for(i=0;i<9;i++)
			{
				awbcheck += HI556_Lgw5m_Sensor_OTP_read(start_address+i);
			}
			r_g_low 		=	HI556_Lgw5m_Sensor_OTP_read(start_address+1);
			r_g_high 		=	HI556_Lgw5m_Sensor_OTP_read(start_address+2);
			b_g_low 		=	HI556_Lgw5m_Sensor_OTP_read(start_address+3);
			b_g_high 		=	HI556_Lgw5m_Sensor_OTP_read(start_address+4);
			r_g_golden_low 	=	HI556_Lgw5m_Sensor_OTP_read(start_address+5);
			r_g_golden_high =	HI556_Lgw5m_Sensor_OTP_read(start_address+6);
			b_g_golden_low 	=	HI556_Lgw5m_Sensor_OTP_read(start_address+7);
			b_g_golden_high =	HI556_Lgw5m_Sensor_OTP_read(start_address+8);
			checksum		=	HI556_Lgw5m_Sensor_OTP_read(start_address+9);
		}
...
#if 1
uint16_t hi556_lgw5m_get_module_id(void)
{
	int index = 0;
	int start_address = 0;
	uint16_t module_id = 0x0;

	HI556OTPLGW5mSetting();
	...
	for(index = 1;index >= 0;index--)
	{
		...
		if(0x55 == HI556_Lgw5m_Sensor_OTP_read(start_address))
		{
			module_id = HI556_Lgw5m_Sensor_OTP_read(start_address+1);
	...
	HI556_OTP_write_cmos_sensor(0x0a00, 0x01);
	return module_id;
}
#endif
...
{    
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	uint16_t hi556_lgw5m_module_id = 0;
...
#if 1
					sensor_init();
					hi556_lgw5m_module_id = read_w5m_moduleid(0x0700);
					if(0x20 != hi556_lgw5m_module_id)
					{
						LOG_INF("hi556_lgw5m_module_id: 0x%x\n", hi556_lgw5m_module_id);
						*sensor_id = 0xFFFFFFFF;
						return ERROR_SENSOR_CONNECT_FAIL;
					}
#endif
					LOG_INF("i2c write id  : 0x%x, sensor id: 0x%x,hi556_lgw5m_module_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id,hi556_lgw5m_module_id);
					return ERROR_NONE;
				}	
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            ...
            break;
		//[LGE_UPDATE_S][bk.bae@lge.com][2018-02-05] update FOV value
		case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
			{
				...
				*pHorFOV = (0x55)<<16 | (0x55); // 16:9 = 85, 4:3 = 85 // (85)<<16 | (85)   //85 degree
				*pVerFOV = (0x31)<<16 | (0x31); // 16:9 = 49, 4:3 = 49 // (49)<<16 | (49)
				*feature_para_len = sizeof(uintptr_t) * 2;
			}
			break;
		//[LGE_UPDATE_E][bk.bae@lge.com][2018-02-05] update FOV value
        default:
            break;
    }
...
UINT32 HI556_MIPI_RAW_LGW5M_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	printk("HI556_MIPI_RAW_LGW5M_SensorInit\n");
