 ...
 * Filename:
 * ---------
 *	 Hi846_mipi_raw_Sensor_front_dongci.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "hi846_dmegc_front_wt_mipi_raw_Sensor.h"

#define PFX "HI846_front_dongci_camera_sensor"
...
static uint16_t hi846_dongci_lsc_flag = 0;
unsigned int hi846_dongci_lsc_addr = 0;
unsigned char hi846_dongci_lsc_data[HI846_DONGCI_LSC_DATA_SIZE];


#define Hi846_FRONT_DONGCI_OTP_FUNCTION    1

#if Hi846_FRONT_DONGCI_OTP_FUNCTION //Hi-846 OPT

//#define RG_Ratio_Typical (0x17B)
//#define BG_Ratio_Typical (0x13C)

void HI846_front_dongci_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 g_gain,kal_uint32 b_gain);
kal_uint16 HI846_front_dongci_Sensor_OTP_read(kal_uint16 otp_addr);
void HI846_front_dongci_Sensor_calc_wbdata(void);
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI846_DMEGC_FRONT_WT_SENSOR_ID,
...
void HI846FRONTDONGCIOTPSetting(void)
{
  LOG_INF("%s enter\n",__func__);
...
#if Hi846_FRONT_DONGCI_OTP_FUNCTION //Hi-846 OPT fuction begin

void Hi846_front_dongci_Sensor_OTP_info(void)
{
	...
	info_flag = HI846_front_dongci_Sensor_OTP_read(0x0201);
	LOG_INF("%s,info_flag = 0x%x\n",__func__,info_flag);
	
	switch (info_flag)
	{
		case 0x01:  //Group1
		ModuleHouseID = HI846_front_dongci_Sensor_OTP_read(0x0202);
		AFFF = HI846_front_dongci_Sensor_OTP_read(0x0203);
		Year = HI846_front_dongci_Sensor_OTP_read(0x0204);
		Month = HI846_front_dongci_Sensor_OTP_read(0x0205);
		Day = HI846_front_dongci_Sensor_OTP_read(0x0206);
		SensorID = HI846_front_dongci_Sensor_OTP_read(0x0207);
		LensID = HI846_front_dongci_Sensor_OTP_read(0x0208);
		VCMID = HI846_front_dongci_Sensor_OTP_read(0x0209);
		DriverID = HI846_front_dongci_Sensor_OTP_read(0x020a);
		FNoID = HI846_front_dongci_Sensor_OTP_read(0x020b);
		infocheck = HI846_front_dongci_Sensor_OTP_read(0x0212);   //020f  20160524
		break;
	case 0x13:
		ModuleHouseID = HI846_front_dongci_Sensor_OTP_read(0x0213);
        AFFF = HI846_front_dongci_Sensor_OTP_read(0x0214);
        Year = HI846_front_dongci_Sensor_OTP_read(0x0215);
        Month = HI846_front_dongci_Sensor_OTP_read(0x0216);
        Day = HI846_front_dongci_Sensor_OTP_read(0x0217);
        SensorID = HI846_front_dongci_Sensor_OTP_read(0x0218);
        LensID = HI846_front_dongci_Sensor_OTP_read(0x0219);
        VCMID = HI846_front_dongci_Sensor_OTP_read(0x021a);
        DriverID = HI846_front_dongci_Sensor_OTP_read(0x021b);
        FNoID = HI846_front_dongci_Sensor_OTP_read(0x021c);
        infocheck = HI846_front_dongci_Sensor_OTP_read(0x0223);     //0220  20160524
        break;
	case 0x37:
        ModuleHouseID = HI846_front_dongci_Sensor_OTP_read(0x0224);
        AFFF = HI846_front_dongci_Sensor_OTP_read(0x0225);
        Year = HI846_front_dongci_Sensor_OTP_read(0x0226);
        Month = HI846_front_dongci_Sensor_OTP_read(0x0227);
        Day = HI846_front_dongci_Sensor_OTP_read(0x0228);
        SensorID = HI846_front_dongci_Sensor_OTP_read(0x0229);
        LensID = HI846_front_dongci_Sensor_OTP_read(0x022a);
        VCMID = HI846_front_dongci_Sensor_OTP_read(0x022b);
        DriverID = HI846_front_dongci_Sensor_OTP_read(0x022c);
        FNoID = HI846_front_dongci_Sensor_OTP_read(0x022d);
        infocheck = HI846_front_dongci_Sensor_OTP_read(0x0234);     //0231  20160524
        break;
    default:
        LOG_INF("HI846_Sensor: info_flag error value: %d \n",info_flag);
        break;
    }
    checksum = (ModuleHouseID + AFFF + Year + Month + Day + SensorID + LensID + VCMID + DriverID + FNoID ) % 0xFF + 1;
    ...
    else
    {
    	...
    	LOG_INF("checksum = %d,infocheck = %d\n", checksum,infocheck);
    }
...
void HI846_front_dongci_Sensor_calc_wbdata(void)
{
	uint16_t wbcheck = 0,checksum = 0,wb_flag = 0,awb_group = 0;
    	uint16_t R_gain=0x0200,B_gain=0x0200,G_gain=0x0200;
  ...
  //uint16_t temp_rg,temp_bg;
	...
	LOG_INF("%s enter\n",__func__);
	wb_flag = HI846_front_dongci_Sensor_OTP_read(0x18B3);
	pr_debug("%s wb_flag = 0x%x\n",__func__,wb_flag);

	switch (wb_flag)
	{
		case 0x01:  //Group1 valid
		    wb_unit_rg_h = HI846_front_dongci_Sensor_OTP_read(0x18B4);
		    wb_unit_rg_l = HI846_front_dongci_Sensor_OTP_read(0x18B5);
		    wb_unit_bg_h = HI846_front_dongci_Sensor_OTP_read(0x18B6);
		    wb_unit_bg_l = HI846_front_dongci_Sensor_OTP_read(0x18B7);
		    wbcheck = HI846_front_dongci_Sensor_OTP_read(0x18B8);
		    awb_group = 1;
			break;
			
		case 0x13:  //Group2 valid
			wb_unit_rg_h = HI846_front_dongci_Sensor_OTP_read(0x18B9);
		    wb_unit_rg_l = HI846_front_dongci_Sensor_OTP_read(0x18BA);
		    wb_unit_bg_h = HI846_front_dongci_Sensor_OTP_read(0x18BB);
		    wb_unit_bg_l = HI846_front_dongci_Sensor_OTP_read(0x18BC);
		    wbcheck = HI846_front_dongci_Sensor_OTP_read(0x18BD);
		    awb_group = 2;
		    break;
		case 0x37:  //Group3 valid
		    wb_unit_rg_h = HI846_front_dongci_Sensor_OTP_read(0x18BE);
		    wb_unit_rg_l = HI846_front_dongci_Sensor_OTP_read(0x18BF);
		    wb_unit_bg_h = HI846_front_dongci_Sensor_OTP_read(0x18C0);
		    wb_unit_bg_l = HI846_front_dongci_Sensor_OTP_read(0x18C1);
		    wbcheck = HI846_front_dongci_Sensor_OTP_read(0x18C2);
		    awb_group = 3;
		    break;
		default:
	...
	LOG_INF("wb_unit_rg_h = 0x%x\n", wb_unit_rg_h);
	LOG_INF("wb_unit_rg_l = 0x%x\n", wb_unit_rg_l);
	LOG_INF("wb_unit_bg_h = 0x%x\n", wb_unit_bg_h);
	LOG_INF("wb_unit_bg_l = 0x%x\n", wb_unit_bg_l);

	// define by module vendor (dongci)
	rg_golden_value = 0x0104;//260
	bg_golden_value = 0x015E;//350

    rg_value = ((wb_unit_rg_h << 8)|wb_unit_rg_l);
    bg_value = ((wb_unit_bg_h << 8)|wb_unit_bg_l);

	LOG_INF("rg_value = 0x%x\n", rg_value);
	LOG_INF("bg_value = 0x%x\n", bg_value);

	R_gain = 0x200 * rg_golden_value/rg_value;
	B_gain = 0x200 * bg_golden_value/bg_value;
...
void Hi846_front_dongci_Sensor_OTP_update_LSC(void)
{
	  ...
	  LOG_INF("%s enter\n",__func__);
		...
		if (HI846_front_dongci_Sensor_OTP_read(0x0235) == 0x01)
			{
				hi846_dongci_lsc_flag = 1;
			}
		else if (HI846_front_dongci_Sensor_OTP_read(0x0235) == 0x13)
			{
				hi846_dongci_lsc_flag = 2;
			}
		else if (HI846_front_dongci_Sensor_OTP_read(0x0235) == 0x37)
			{
				hi846_dongci_lsc_flag = 3;
			}
	  LOG_INF("%s,lsc_flag = 0x%x\n",__func__,hi846_dongci_lsc_flag);
	  //temp = read_cmos_sensor (0x0a05) | 0x04;
	  //LOG_INF("%s,lsc_enable flag = 0x%x\n",__func__,temp);
	  //HI846_OTP_write_cmos_sensor(0x0a05, temp); //LSC enable
		if(hi846_dongci_lsc_flag == 1)
			hi846_dongci_lsc_addr = 0x0236;
		else if(hi846_dongci_lsc_flag == 2)
			hi846_dongci_lsc_addr = 0x09B5;
		else if(hi846_dongci_lsc_flag ==3)
			hi846_dongci_lsc_addr = 0x1134;
		for (loop = 0; loop < HI846_DONGCI_LSC_DATA_SIZE; loop++){
			tt = HI846_front_dongci_Sensor_OTP_read(hi846_dongci_lsc_addr);
			hi846_dongci_lsc_data[loop]=tt;
			hi846_dongci_lsc_addr++;
			}
	  LOG_INF("%s exit\n",__func__);
}


uint16_t hi846_front_dongci_get_module_id(void)
{
	uint16_t ModuleHouseID = 0;
	uint16_t info_flag = 0;

	HI846FRONTDONGCIOTPSetting();
	...
	Hi846_front_dongci_Sensor_OTP_update_LSC();////read lsc data
	HI846_OTP_write_cmos_sensor(0x0a00, 0x00);
	mdelay(10);
	HI846_OTP_write_cmos_sensor(0x003e, 0x00);
	HI846_OTP_write_cmos_sensor(0x0a00, 0x00);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	uint16_t hi846_dongci_module_id = 0;
...
#if 1
				sensor_init();
				hi846_dongci_module_id = hi846_front_dongci_get_module_id();
				printk("hi846_dongci_module_id: 0x%x\n", hi846_dongci_module_id);
				if(0x13 != hi846_dongci_module_id)
				{
...
#if 0//Hi846_FRONT_DONGCI_OTP_FUNCTION
    HI846FRONTDONGCIOTPSetting();
    Hi846_front_dongci_Sensor_OTP_info();
    HI846_front_dongci_Sensor_calc_wbdata();
#endif
		...
		case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
			{
				...
				*pHorFOV = (0x50)<<16 | (0x50); // 16:9 = 64, 4:3 = 64 // (64)<<16 | (64)   //64 degree
...
UINT32 HI846_DMEGC_FRONT_WT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
