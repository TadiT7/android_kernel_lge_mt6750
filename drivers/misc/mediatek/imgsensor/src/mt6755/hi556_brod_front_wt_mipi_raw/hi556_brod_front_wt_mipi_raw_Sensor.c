 ...
 * Filename:
 * ---------
 *	 hi556_brod_front_wt_mipi_raw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
 ...
 *       info : 5M   front camera  module vendor: broad (bolixin)  1st
 *
 *------------------------------------------------------------------------------
...
#include "hi556_brod_front_wt_mipi_raw_Sensor.h"

#define PFX "hi556_front_bolixin_camera_sensor"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define Hi556_OTP_FUNCTION 0

#if Hi556_OTP_FUNCTION //Hi-556 OPT
void HI556_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 g_gain,kal_uint32 b_gain);
kal_uint16 HI556_Sensor_OTP_read(kal_uint16 otp_addr);
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = (HI556_BROD_FRONT_WT_SENSOR_ID - 0x4),
	...
	.i2c_addr_table = {0x50,0xff},//{0x40,0xff},
};
...
void HI556OTPSetting(void)
{
	...
	HI556_OTP_write_cmos_sensor(0x011a, 0x01);//CP TRIM_H
	HI556_OTP_write_cmos_sensor(0x011b, 0x09);//IPGM TRIM_H
	HI556_OTP_write_cmos_sensor(0x0d04, 0x01);//Fsync(OTP busy)Output Enable
	HI556_OTP_write_cmos_sensor(0x0d00, 0x07);//Fsync(OTP busy)Output Drivability
	HI556_OTP_write_cmos_sensor(0x003e, 0x10);//OTP r/w mode
	HI556_OTP_write_cmos_sensor(0x0a00, 0x01);//standby off
...
kal_uint16 HI556_Sensor_OTP_read(kal_uint16 otp_addr)
{
	...
	HI556_OTP_write_cmos_sensor(0x010a, (i >> 8) & 0xFF); //start address H
	HI556_OTP_write_cmos_sensor(0x010b, i & 0xFF); //start address L
	HI556_OTP_write_cmos_sensor(0x0102, 0x01); //read enable
	data = read_cmos_sensor_byte(0x0108); //OTP data read
    //data = read_cmos_sensor(0x0708); //OTP data read
	return data;
}

#if Hi556_OTP_FUNCTION //Hi-556 OPT fuction begin
...
void HI556_Sensor_OTP_info(void)
{
	...
	{
		start_address = index*8 + 0x0401;
		if(0x55 == HI556_Sensor_OTP_read(start_address))
		{
			for(i=0;i<7;i++)
			{
				infocheck += HI556_Sensor_OTP_read(start_address+i);
			}
			ModuleID = HI556_Sensor_OTP_read(start_address+1);
			LensID = HI556_Sensor_OTP_read(start_address+2);
			VCMID = HI556_Sensor_OTP_read(start_address+3);
			Year = HI556_Sensor_OTP_read(start_address+4);
			Month = HI556_Sensor_OTP_read(start_address+5);
			Day = HI556_Sensor_OTP_read(start_address+6);
			checksum = HI556_Sensor_OTP_read(start_address+7);
		}
		LOG_INF("HI556_Sensor:infocheck = 0x%x checksum = 0x%x \n",infocheck,checksum);

		if(checksum == ((infocheck%0xff) + 1))
		{
...
void HI556_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 g_gain,kal_uint32 b_gain)
{
	pr_debug("%s,r_gain = 0x%x, b_gain = 0x%x ,g_gain = 0x%x\n", __func__,r_gain, b_gain ,g_gain);

	HI556_OTP_write_cmos_sensor(0x0078, g_gain >> 8); //gr_gain
	HI556_OTP_write_cmos_sensor(0x0079, g_gain & 0xFF); //gr_gain
	HI556_OTP_write_cmos_sensor(0x007a, g_gain >> 8); //gb_gain
	HI556_OTP_write_cmos_sensor(0x007b, g_gain & 0xFF); //gb_gain

	HI556_OTP_write_cmos_sensor(0x007c, r_gain >> 8); //r_gain
	HI556_OTP_write_cmos_sensor(0x007d, r_gain & 0xFF); //r_gain
	HI556_OTP_write_cmos_sensor(0x007e, b_gain >> 8); //b_gain
	HI556_OTP_write_cmos_sensor(0x007f, b_gain & 0xFF); //b_gain
}

void HI556_Sensor_calc_wbdata(void)
{
	int RGain=0,BGain=0,RGain_Golden=0,BGain_Golden=0;
	int r_g_low=0,r_g_high=0,b_g_low=0,b_g_high=0;
	int r_g_golden_low=0,r_g_golden_high=0,b_g_golden_low=0,b_g_golden_high=0;
	int R_gain= 0x0100, G_gain= 0x0100, B_gain= 0x0100;
	int awbcheck=0,checksum=0;
	int i,index,start_address;
	int awb_group = -1;

	for(index = 0;index < 3;index++)
	{
		start_address = index*10 + 0x0419;
		if(0x55 == HI556_Sensor_OTP_read(start_address))
		{
			for(i=0;i<9;i++)
			{
				awbcheck += HI556_Sensor_OTP_read(start_address+i);
			}
			r_g_low 		=	HI556_Sensor_OTP_read(start_address+1);
			r_g_high 		=	HI556_Sensor_OTP_read(start_address+2);
			b_g_low 		=	HI556_Sensor_OTP_read(start_address+3);
			b_g_high 		=	HI556_Sensor_OTP_read(start_address+4);
			r_g_golden_low 	=	HI556_Sensor_OTP_read(start_address+5);
			r_g_golden_high =	HI556_Sensor_OTP_read(start_address+6);
			b_g_golden_low 	=	HI556_Sensor_OTP_read(start_address+7);
			b_g_golden_high =	HI556_Sensor_OTP_read(start_address+8);
			checksum		=	HI556_Sensor_OTP_read(start_address+9);
		}
		LOG_INF("HI556_Sensor: awbcheck = 0x%x checksum = 0x%x\n",awbcheck,checksum);
		if(checksum == ((awbcheck%0xff) + 1))
		{
			LOG_INF("HI556_Sensor: awb data checksum PASS \n");
			awb_group = index + 1;
			break;
		}
		else
		{
			LOG_INF("HI556_Sensor: awb data checksum Fail \n");
		}
	}
	...
	/*
	WB update
	In the design house, they use above data for WB calibration.
	We need correction like followings to make minimum gain to be higher than 0x100.
	*/

	R_gain = 0x100 * RGain_Golden/RGain;
	B_gain = 0x100 * BGain_Golden/BGain;
    ...
    HI556_OTP_write_cmos_sensor(0x0a00, 0x00);
    mdelay(10);
    HI556_OTP_write_cmos_sensor(0x003e, 0x00);
    HI556_OTP_write_cmos_sensor(0x0a00, 0x00);
    HI556_Sensor_update_wb_gain(R_gain, G_gain,B_gain);
	LOG_INF("%s exit\n",__func__);
}
#endif //Hi-556 OTP function finish
#if 1
#define HI556_FRONT_BOLIXIN_AWB_DATA_SIZE    11
unsigned char Hi556_front_bolixin_awb_data[HI556_FRONT_BOLIXIN_AWB_DATA_SIZE];

#define HI556_FRONT_BOLIXIN_LSC_DATA_SIZE    1868
unsigned char hi556_front_bolixin_lsc_data[HI556_FRONT_BOLIXIN_LSC_DATA_SIZE];

int hi556_front_bolixin_Awb_Sensor_OTP_update(void)
{
	kal_uint16 tt = 0;
	kal_uint16 loop = 0;
	kal_uint16 checksum = 0x55;
	kal_uint16 Hi556_front_bolixin_awb_flag = 0;
	kal_uint16 Hi556_front_bolixin_awb_addr = 0;

	LOG_INF("%s enter\n",__func__);
	if (HI556_Sensor_OTP_read(0x0431) == 0x55)
		{
			Hi556_front_bolixin_awb_flag = 3;
		}
	else if (HI556_Sensor_OTP_read(0x0425) == 0x55)
		{
			Hi556_front_bolixin_awb_flag = 2;
		}
	else if (HI556_Sensor_OTP_read(0x0419) == 0x55)
		{
			Hi556_front_bolixin_awb_flag = 1;
		}
	LOG_INF("%s,awb_flag = 0x%x\n",__func__,Hi556_front_bolixin_awb_flag);
	if(Hi556_front_bolixin_awb_flag == 1)
		Hi556_front_bolixin_awb_addr = 0x041A;
	else if(Hi556_front_bolixin_awb_flag == 2)
		Hi556_front_bolixin_awb_addr = 0x0426;
	else if(Hi556_front_bolixin_awb_flag == 3)
		Hi556_front_bolixin_awb_addr = 0x0432;

	for (loop = 0; loop < HI556_FRONT_BOLIXIN_AWB_DATA_SIZE; loop++){
		tt = HI556_Sensor_OTP_read(Hi556_front_bolixin_awb_addr);
		Hi556_front_bolixin_awb_data[loop]=tt;
		Hi556_front_bolixin_awb_addr++;
	}
	for(loop = 0;loop < HI556_FRONT_BOLIXIN_AWB_DATA_SIZE-1;loop++)
	{
		LOG_INF("%s: Hi556_front_bolixin_awb_data[%d]:0x%x\n",__func__,loop,Hi556_front_bolixin_awb_data[loop]);
		checksum += Hi556_front_bolixin_awb_data[loop];
	}
	LOG_INF("%s:Hi556_front_bolixin_awb_data[10] = 0x%x checksum = 0x%x,0x%x\n",__func__,Hi556_front_bolixin_awb_data[10],checksum,((checksum%0xff) + 1));
	if(((checksum%0xff) + 1) ==  Hi556_front_bolixin_awb_data[10])
	{
...
int hi556_front_bolixin_lsc_Sensor_OTP_update_LSC(void)
{
	kal_uint16 tt = 0;
	kal_uint16 loop = 0;
	kal_uint32 checksum = 0x55;
	kal_uint16 checksum_value = 0;
	kal_uint16 hi556_bolixin_lsc_checksum_addr = 0;
	kal_uint16 hi556_bolixin_lsc_flag = 0;
	kal_uint16 hi556_bolixin_lsc_addr = 0;

	LOG_INF("%s enter\n",__func__);

	if (HI556_Sensor_OTP_read(0x012D9) == 0x55)
	{
		hi556_bolixin_lsc_flag = 3;
	}
	else if (HI556_Sensor_OTP_read(0x0B8B) == 0x55)
	{
		hi556_bolixin_lsc_flag = 2;
	}
	else if (HI556_Sensor_OTP_read(0x043D) == 0x55)
	{
		hi556_bolixin_lsc_flag = 1;
	}
	LOG_INF("%s,lsc_flag = 0x%x\n",__func__,hi556_bolixin_lsc_flag);
	if(hi556_bolixin_lsc_flag == 1)
	{
		hi556_bolixin_lsc_addr = 0x043E;
		hi556_bolixin_lsc_checksum_addr = 0x0B8A;
	}
	else if(hi556_bolixin_lsc_flag == 2)
	{
		hi556_bolixin_lsc_addr = 0x0B8C;
		hi556_bolixin_lsc_checksum_addr = 0x012D8;
	}
	else if(hi556_bolixin_lsc_flag == 3)
	{
		hi556_bolixin_lsc_addr = 0x012DA;
		hi556_bolixin_lsc_checksum_addr = 0x1A26;
	}
	for (loop = 0; loop < HI556_FRONT_BOLIXIN_LSC_DATA_SIZE; loop++){
		tt = HI556_Sensor_OTP_read(hi556_bolixin_lsc_addr);
		hi556_front_bolixin_lsc_data[loop]=tt;
		hi556_bolixin_lsc_addr++;
	}
	for(loop = 0;loop < HI556_FRONT_BOLIXIN_LSC_DATA_SIZE;loop++)
	{
		LOG_INF("%s,hi556_front_bolixin_lsc_data[%d] = 0x%x\n",__func__,loop,hi556_front_bolixin_lsc_data[loop]);
		checksum += hi556_front_bolixin_lsc_data[loop];
	}
	checksum_value = HI556_Sensor_OTP_read(hi556_bolixin_lsc_checksum_addr);
...
uint16_t hi556_get_module_id(void)
{
	int index = 0;
	int start_address = 0;
	uint16_t module_id = 0x0;
	int ret = -1;
	...
	for(index = 2;index >= 0;index--)
	{
		...
		if(0x55 == HI556_Sensor_OTP_read(start_address))
		{
			module_id = HI556_Sensor_OTP_read(start_address+1);
			LOG_INF("%s module_id: 0x%x,index: %d\n",__func__,module_id,index);
			break;
		}
	}
#if 1
	ret = hi556_front_bolixin_Awb_Sensor_OTP_update();
	if(ret == 0)
		return ret;// checksum fail return error
	ret = hi556_front_bolixin_lsc_Sensor_OTP_update_LSC();
	if(ret == 0)
		return ret;// checksum fail return error
#endif
	HI556_OTP_write_cmos_sensor(0x0a00, 0x00);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id =return_sensor_id();
				printk("hi556 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				if (*sensor_id == imgsensor_info.sensor_id) {
#if 1
					sensor_init();
					hi556_front_module_id = hi556_get_module_id();
					if(0x0E != hi556_front_module_id)
					{
						printk("OTP Checksum Fail! or hi556_front_module_id:0x%x\n",hi556_front_module_id);
						// if module ID is not correct, Must set *sensor_id to 0xFFFFFFFF for exit
						//*sensor_id = 0xFFFFFFFF;
						//return ERROR_SENSOR_CONNECT_FAIL;
						printk("[HI556_BROD] vendor id is different!! you should check the EEPROM or Module hardware\n");
					}
#endif
...
static kal_uint32 open(void)
{
	...
	LOG_INF("[open]: PLATFORM:MT6750,MIPI 2LANE\n");
...
UINT32 HI556_BROD_FRONT_WT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    printk("HI556_BROD_FRONT_WT_MIPI_RAW_SensorInit\n");
