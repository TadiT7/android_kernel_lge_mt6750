...
#include "hi846_sunny_mipi_raw_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_SUNNY_SENSOR_ID - 0x2000,
	...
	.margin = 6,
	.min_shutter = 2, //6, //[jk.ra@lge.com] [2017-09-28] modified minimum line count for HI846.
...
// [LGE_UPDATE_S] [kyunghun.oh@lge.com] [2017-09-18] Creation of node for camera module identification.
static int hi846_sunny_read_eepromVendorID(void)
{
    int retry = 3;
    u8 vendorID_readFrom_rear_imageSensor = 0;
    for(; retry > 0; retry--) {
        if(iReadReg(0x0700, &vendorID_readFrom_rear_imageSensor, 0xa0) == 0)
            break;
        mdelay(1);
    }
    printk("[hi846_sunny_read_eepromVendorID] read eeprom id = %d\n", vendorID_readFrom_rear_imageSensor);
    if(vendorID_readFrom_rear_imageSensor != 0x20)
    {
        printk("[hi846_sunny_read_eepromVendorID] matching fail!! It should be 0x01 but read %d\n", vendorID_readFrom_rear_imageSensor);
        return -1;
    }
    else
    {
        printk("[hi846_sunny_read_eepromVendorID] matching OK\n");
        strcpy(rear_sensor_name, "sl846_sunny");
        return 0;
    }
}
// [LGE_UPDATE_E] [kyunghun.oh@lge.com] [2017-09-18] Creation of node for camera module identification.
...
// [LGE_UPDATE_S] [kyunghun.oh@lge.com] [2017-09-18] Creation of node for camera module identification.
                    if(hi846_sunny_read_eepromVendorID() != 0)
                        printk("[HI846_SUNNY] vendor id is different!! you should check the EEPROM or Module hardware\n");
        ...
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
            {
                ...
                *pHorFOV = (0x3A)<<16 | (0x3A); // 16:9 = 58, 4:3 = 58 // (58)<<16 | (58)   //58 degree
...
UINT32 HI846_SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
