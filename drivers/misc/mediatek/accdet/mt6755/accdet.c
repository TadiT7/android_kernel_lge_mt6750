...
#ifdef CONFIG_LGE_ACCDET_SANITY
static ssize_t lge_accdet_sanity_read_proc(struct file *file, char __user *buf,
											size_t nbytes, loff_t *offp)
{
	char *page;
	ssize_t	n = 0;

	printk( KERN_INFO "accdet_read_proc : Enter %zu, %lld\n", nbytes, *offp);
	if(offp == NULL){
		printk(KERN_INFO "%s offp pointer is null\n", __func__);
		return 0;
	}

	if(buf == NULL){
		printk(KERN_INFO "%s buf pointer is null\n", __func__);
		return 0;
	}

	if(*offp){
		printk(KERN_INFO "%s File position is not zero before read\n", __func__);
		return 0;
	}

	if (!(page = (char*) __get_free_page(GFP_TEMPORARY))){
		printk(KERN_INFO "%s Can not get page\n", __func__);
		return -ENOMEM;
	}

	if(file == NULL){
		printk(KERN_INFO "%s file pointer is null\n", __func__);
		n = sprintf(page, "File pointer is null");
		goto end;
	}

	switch(accdet_data.state){
		case NO_DEVICE:
			n = sprintf(page, "No Device");
			break;
		case HEADSET_MIC:
			n = sprintf(page, "Headset with Mic");
			break;
		case HEADSET_NO_MIC:
			n = sprintf(page, "Headset without Mic");
			break;
		default:
			n = sprintf(page, "Input state is wrong");
			break;
	}

end:
	printk( KERN_INFO "accdet_read_proc : %s\n", page);

	if(n>0){
		ssize_t left;
		page[n] = -1;
		n++;
		left = copy_to_user(buf, page, n);
		n -= left;
	}
...
static ssize_t lge_accdet_sanity_write_proc(struct file *file,
												...
												size_t count, loff_t *offp)
{
	...
	if(d == accdet_data.state){
		goto end;
	}

	switch(d){
		case NO_DEVICE:
			printk(KERN_INFO "accdet_write_proc : NO_DEVICE, current : %d\n",
					accdet_data.state);
		...
		case HEADSET_MIC:
			printk(KERN_INFO "accdet_write_proc : HEADSET_WITH_MIC, current : %d\n",
					accdet_data.state);
		...
		case HEADSET_NO_MIC:
			printk( KERN_INFO "accdet_write_proc : HEADSET_NO_MIC, current : %d\n",
					accdet_data.state);
		...
		case HEADSET_HOOK:
			printk( KERN_INFO "accdet_write_proc : HOOK, current : %d\n",
					accdet_data.state);
...
static const struct file_operations proc_accdet_sanity_operations = {
	.read		= lge_accdet_sanity_read_proc,
	.write		= lge_accdet_sanity_write_proc
};

struct proc_dir_entry *proc_accdet_sanity_entry;

static void lge_accdet_sanity_init(void)
{
	proc_accdet_sanity_entry = proc_create_data("accdet_sanity_ctrl",
									S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH,
									NULL,
									&proc_accdet_sanity_operations,
									NULL);
}
#endif
