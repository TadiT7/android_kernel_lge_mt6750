...
#define REF_HIGH_TEMP 50
#define mtkts_bts_printk(fmt, args...)   \
do {                                    \
	if (mtkts_bts_debug_log) {                \
		pr_err("[Display/BTS_Thermal]" fmt, ##args); \
	}                                   \
} while (0)

static int mtkts_bts_set_flag = 0;
extern void primary_display_set_surface_temp(unsigned int enable);
#endif
...
#ifdef CONFIG_LGE_CHECK_SURFACE_TEMPERATURE
int mtkts_bts_get_debug_log(void)
{
	return mtkts_bts_debug_log;
}

void mtkts_bts_set_debug_log(int enable)
{
	mtkts_bts_debug_log = enable;
}

void mtkts_bts_set_gamma_cmd(int cur_temp)
{
    if(bts_cur_temp <= 0)
        return;
    ...
    if(cur_temp > REF_HIGH_TEMP && !mtkts_bts_set_flag) {
        mtkts_bts_set_flag = 1;
        primary_display_set_surface_temp(mtkts_bts_set_flag);
        mtkts_bts_printk("Normal => High temp (%d)\n",cur_temp);
    } else if(cur_temp <= REF_HIGH_TEMP && mtkts_bts_set_flag) {
        mtkts_bts_set_flag = 0;
        primary_display_set_surface_temp(mtkts_bts_set_flag);
        mtkts_bts_printk("High temp => Normal (%d)\n",cur_temp);
    } else {
        mtkts_bts_printk("cur_temp = %d, maintain %s state\n", cur_temp, mtkts_bts_set_flag? "High temperature":"Normal");
    }
}
#endif
