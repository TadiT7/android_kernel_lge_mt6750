...
struct mtk_thermal_cooler_data {
	...
	int exit_threshold;
	int id;
#ifdef CONFIG_LGE_PM_THERMAL_UPPER_THRE
	int extra_cond;
#endif
};
...
#ifdef CONFIG_LGE_PM_THERMAL_UPPER_THRE
static unsigned long long g_check_upper_exit_flag = 0x0;

static int nThermalzone_temp;
#endif
...
#ifdef CONFIG_LGE_PM_THERMAL_UPPER_THRE
	THERMAL_SYS_INFO_UPPER_EXIT = 0x40,
#endif
...
#ifdef CONFIG_LGE_PM_THERMAL_UPPER_THRE
		mask |= (g_check_upper_exit_flag == 0ULL) ? 0 : THERMAL_SYS_INFO_UPPER_EXIT;
#endif
...
#ifdef CONFIG_LGE_PM_THERMAL_UPPER_THRE
				} else if (0 == strncmp(cldata->conditions[i], "UPPER", 5)) {
					nThermalzone_temp = cldata->tz->temperature;
					if (*cldata->condition_last_value[i] < cldata->threshold[i])
						ret++;
#endif
...
#ifdef CONFIG_LGE_PM_THERMAL_UPPER_THRE
	g_check_upper_exit_flag &= (~(1ULL << cldata->id));
#endif
...
#ifdef CONFIG_LGE_PM_THERMAL_UPPER_THRE
			} else if (0 == strncmp(mcdata->conditions[i], "UPPER", 5)){
				mcdata->condition_last_value[i] = &nThermalzone_temp;
				g_check_upper_exit_flag |= (1ULL << mcdata->id);
				mcdata->extra_cond = 1;
				THRML_LOG("%s upper exit flag: %016llx, id=%d\n", __func__,
					g_check_upper_exit_flag, mcdata->id);
#endif
			} else {
