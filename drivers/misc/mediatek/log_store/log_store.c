...
{
	if (!sram_header) {
		pr_err("%s: sram_headser is null..\n", __func__);
		return 0;
	}

	if (!(sram_header->reserve[0]))
		return 0;
	...
	if (!last_pl_lk)
		last_pl_lk = remap_lowmem(sram_header->reserve[0], (unsigned int)LOG_STORE_SIZE_MIRROR);

	if ((dram_log_store_status == BUFF_READY) && sram_header
		&& (sram_header->sig == SRAM_HEADER_SIG)) {
		if (last_pl_lk) {
			if (!check_magic(last_pl_lk)) {
