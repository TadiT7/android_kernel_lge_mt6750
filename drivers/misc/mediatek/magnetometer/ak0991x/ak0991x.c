...
struct mag_hw mag_cust_m;
static struct mag_hw *hw = &mag_cust_m;


static int ak0991x_local_init(void);
...
static struct mag_init_info ak0991x_init_info = {
	.name = "ak0991x",
	.init = ak0991x_local_init,
	.uninit = ak0991x_remove,
};

static int ak0991x_init_flag;
...
static int akecs_checkdevice(struct ak0991x_data *akm)
{
	...
	case AK09918_DEVICE_ID:
		dev_err(akm->dev, "%s: device_id-%d\n", __func__, akm->company_id);
		/* AK0991x: nop */
		break;
	default:
		/* Other: error */
...
static int akecs_read_fuse(struct ak0991x_data *akm)
{
	...
	if ((AK09913_DEVICE_ID == akm->device_id) ||
	    ...
	    (AK09918_DEVICE_ID == akm->device_id)) {
...
static int read_and_event(struct ak0991x_data *akm)
{
	...
	reg_ST1[0] = AK0991X_ADDR_ST1;
	err = akm->bops->rxdata(akm->dev, &reg_ST1[0], 1);
	if (err)
		return err;

	/* Check ST bit */
	if ((reg_ST1[0] & AK0991X_DATA_READY) != AK0991X_DATA_READY)
		dev_err(akm->dev, "%s: DRDY is not set", __func__);
	//	return -EAGAIN;

	buffer[0] = AK0991X_ADDR_HXL;
	err = akm->bops->rxdata(akm->dev, buffer, 8);
	if (err)
		return err;
	...
	input_event(akm->input, EV_ABS, EVENT_TYPE_MAGEL_X, val32[0]);
	input_event(akm->input, EV_ABS, EVENT_TYPE_MAGEL_Y, val32[1]);
	input_event(akm->input, EV_ABS, EVENT_TYPE_MAGEL_Z, val32[2]);
	input_event(akm->input, EV_ABS, EVENT_TYPE_MAGEL_STATUS, val_st2);
//	input_event(akm->input, EV_MSC, MSC_TSL, val_tsl);
//	input_event(akm->input, EV_MSC, MSC_TSH, val_tsh);

	input_event(akm->input, EV_ABS, EVENT_TYPE_O_X, val32[0]);//val32[0] is dummy data
	input_event(akm->input, EV_ABS, EVENT_TYPE_O_Y, val32[1]);//val32[1] is dummy data
	input_event(akm->input, EV_ABS, EVENT_TYPE_O_Z, val32[2]);//val32[2] is dummy data
	input_event(akm->input, EV_ABS, EVENT_TYPE_O_STATUS, val_st2);//val_st2 is dummy data
				   ...
				   const char *buf, size_t count)
{
	...
	dev_dbg(dev, "%s called: '%s'", __func__, buf);
...
//	int mag_interval_tmp;
	...
	if (0 > interval) {
		...
		atomic_set(&akm->mode_rsv, AK0991X_CNTL_CNT(AK0991X_CNT_FREQ_100HZ));
		next_interval = 10;
		atomic_set(&akm->interval_rsv, 10);
	} else if (50 > interval) {
		next_mode = AK0991X_CNTL_CNT(AK0991X_CNT_FREQ_50HZ);
		atomic_set(&akm->mode_rsv, AK0991X_CNTL_CNT(AK0991X_CNT_FREQ_50HZ));
		next_interval = 20;
		atomic_set(&akm->interval_rsv, 20);
	} else if (100 > interval) {
		next_mode = AK0991X_CNTL_CNT(AK0991X_CNT_FREQ_20HZ);
		atomic_set(&akm->mode_rsv, AK0991X_CNTL_CNT(AK0991X_CNT_FREQ_20HZ));
		next_interval = 50;
		atomic_set(&akm->interval_rsv, 50);
	} else {
		/* the slowest */
		next_mode = AK0991X_CNTL_CNT(AK0991X_CNT_FREQ_10HZ);
		atomic_set(&akm->mode_rsv, AK0991X_CNTL_CNT(AK0991X_CNT_FREQ_10HZ));
		next_interval = 100;
		atomic_set(&akm->interval_rsv, 100);
	}
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	dev_dbg(dev, "%s called", __func__);
	return scnprintf(buf, PAGE_SIZE, "%d", akecs_busy_check(akm));
}
				...
				const char *buf, size_t count)
{
	...
	dev_dbg(dev, "%s called: '%s'", __func__, buf);
	if (!strncmp(buf, "1", 1)) {
		next_mode = atomic_read(&akm->mode_rsv);
		next_interval = atomic_read(&akm->interval_rsv);
		err = akecs_setmode_measure(akm, next_mode, next_interval);
		if (err < 0) {
			dev_dbg(dev, "%s called: setmode error\n", __func__);
			return err;
		}
		...
		dev_dbg(dev, "%s called: error\n", __func__);
	}

	dev_dbg(dev, "%s called: success\n", __func__);
	return count;
}
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	devname = dev_name(&akm->input->dev);
...
//	__ATTR(continuous, 0220, NULL,               attr_continuous_store),
	__ATTR(magdelay,   0220,	      NULL,		     attr_continuous_store),
	__ATTR(reset,	   0220,	      NULL,		     attr_reset_store),
	__ATTR(magdevnum,  S_IWUSR | S_IRUGO, mag_show_sensordevnum, NULL),
	__ATTR(magactive,  S_IWUSR | S_IRUGO, mag_show_active,	     mag_store_active),
#ifdef DEBUG
				   ...
				   const struct ak0991x_bus_ops *bops)
{
	...
	err = hwmsen_get_convert(hw->direction, &cvt);
	if (err) {
		dev_err(dev, "%s: axis setting failed.", __func__);
		goto err_axis;
	}

	for (i = 0; i < 3; i++) {
		akm->axis_order[i] = cvt.map[i];
		akm->axis_sign[i] = ((cvt.sign[i] == 1) ? 0 : 1);
	}
	...
	input_set_capability(akm->input, EV_ABS, EVENT_TYPE_MAGEL_X);
	input_set_capability(akm->input, EV_ABS, EVENT_TYPE_MAGEL_Y);
	input_set_capability(akm->input, EV_ABS, EVENT_TYPE_MAGEL_Z);
	input_set_capability(akm->input, EV_ABS, EVENT_TYPE_MAGEL_STATUS);
	input_set_capability(akm->input, EV_ABS, MSC_TSL);
	input_set_capability(akm->input, EV_ABS, MSC_TSH);

	input_set_capability(akm->input, EV_ABS, EVENT_TYPE_O_X);
	input_set_capability(akm->input, EV_ABS, EVENT_TYPE_O_Y);
	input_set_capability(akm->input, EV_ABS, EVENT_TYPE_O_Z);
	input_set_capability(akm->input, EV_ABS, EVENT_TYPE_O_STATUS);
	...
	akm->mdev.minor = MISC_DYNAMIC_MINOR;
	akm->mdev.name = MAG_MISC_DEV_NAME;

	err = misc_register(&akm->mdev);
	if (err) {
		dev_err(dev, "ak0991x_device_register failed\n");
		goto err_misc;
	}
...
err_power_on:
	kfree(akm);
err_kzalloc:
	dev_info(dev, "%s: Error!!.", __func__);
	ak0991x_init_flag = -1;
	return ERR_PTR(err);
}
EXPORT_SYMBOL_GPL(ak0991x_probe);

int ak0991x_remove(void)
{
...
static int ak0991x_i2c_remove(struct i2c_client *client)
{
	...
	//i2c_unregister_device(akm);
	i2c_unregister_device(client);
	kfree(akm);
	misc_deregister(&akm->mdev);
	return 0;
}
...
*/
	mag_driver_add(&ak0991x_init_info);
