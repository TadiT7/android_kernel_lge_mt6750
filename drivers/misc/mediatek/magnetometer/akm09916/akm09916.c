...
static long AKECS_Reset(int hard)
{
	...
	if (hard != 0) {
		...
		if (err < 0)
			MAG_ERR("%s: Can not set SRST bit.\n", __func__);
		else
	  ...
	  const char testname[], const int testdata, const int lolimit, const int hilimit, int *pf_total)
{
	...
	if ((testno == NULL) && (strncmp(testname, "START", 5) == 0)) {
		...
		if (*pf_total == 1)
			...
			MAG_ERR("Factory shipment test was failed.\n\n");
...
static ssize_t show_shipment_test(struct device_driver *ddri, char *buf)
{
	...
	if (1 == res) {
		...
		MAG_ERR("shipment_test fail\n");
		strcpy(result, "n");
	} else {
		MAG_ERR("shipment_test NaN\n");
		strcpy(result, "NaN");
	}
...
static ssize_t store_layout_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (err == 0) {
		...
		err = hwmsen_get_convert(layout, &data->cvt);
		if (err) {
			MAG_ERR("hwmsen_get_convert(layout, &data->cvt) failed !\n");

			err = hwmsen_get_convert(data->hw->direction, &data->cvt);
			if (err) {
				MAG_ERR("hwmsen_get_convert(data->hw->direction, &data->cvt) failed !\n");

				hwmsen_get_convert(0, &data->cvt);
				MAG_ERR("invalid layout: %d, restore to 0\n", layout);
			} else {
				MAGN_LOG("hwmsen_get_convert(data->hw->direction, &data->cvt) success !\n");
				MAG_ERR("invalid layout: %d, restore to %d\n", layout, data->hw->direction);
			}
		} else {
			MAGN_LOG("hwmsen_get_convert(layout, &data->cvt) success !\n");
		}
	} else {
...
static DRIVER_ATTR(orientation, S_IRUGO | S_IWUSR, show_chip_orientation, store_chip_orientation);
...
static long akm09916_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_WRITE:
		MAGN_LOG("ECS_IOCTL_WRITE\n");
		if (argp == NULL) {
			MAG_ERR("invalid argument.\n");
			return -EINVAL;
		}
		if (copy_from_user(rwbuf, argp, sizeof(rwbuf))) {
			MAG_ERR("copy_from_user failed.\n");
			return -EFAULT;
		}
	...
	case ECS_IOCTL_READ:
		...
		if (copy_to_user(argp, rwbuf, rwbuf[0] + 1)) {
			MAG_ERR("copy_to_user failed.\n");
			return -EFAULT;
		}
		break;

	case ECS_IOCTL_GET_INFO:
		MAGN_LOG("ECS_IOCTL_GET_INFO\n");
#ifdef AKM_Device_AK8963
	...
	case ECS_IOCTL_SET_YPR_09911:
		MAGN_LOG("ECS_IOCTL_SET_YPR_09911\n");
		if (argp == NULL) {
			MAG_ERR("invalid argument.\n");
			return -EINVAL;
		}
	...
	case ECS_IOCTL_GET_OPEN_STATUS:
		MAGN_LOG("ECS_IOCTL_GET_OPEN_STATUS\n");
	...
	case ECS_IOCTL_GET_CLOSE_STATUS:
		MAGN_LOG("ECS_IOCTL_GET_CLOSE_STATUS\n");
	...
	case ECS_IOCTL_GET_OSENSOR_STATUS:
		MAGN_LOG("ECS_IOCTL_GET_DELAY_09911\n");
	...
	case ECS_IOCTL_GET_LAYOUT_09911:
		MAGN_LOG("ECS_IOCTL_GET_LAYOUT_09911\n");
	...
	case MSENSOR_IOCTL_READ_CHIPINFO:
		MAGN_LOG("MSENSOR_IOCTL_READ_CHIPINFO\n");
		if (argp == NULL) {
	...
	case MSENSOR_IOCTL_READ_SENSORDATA:
		MAGN_LOG("MSENSOR_IOCTL_READ_SENSORDATA\n");
		if (argp == NULL) {
			MAG_ERR("IO parameter pointer is NULL!\n");
			break;
		}
	...
	case MSENSOR_IOCTL_SENSOR_ENABLE:
		MAGN_LOG("MSENSOR_IOCTL_SENSOR_ENABLE\n");
	...
	case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
		MAGN_LOG("MSENSOR_IOCTL_READ_FACTORY_SENSORDATA\n");
		if (argp == NULL) {
			MAG_ERR("IO parameter pointer is NULL!\n");
			break;
		}
	...
	default:
		MAG_ERR("%s not supported = 0x%04x\n", __func__, cmd);
		ret = -ENOIOCTLCMD;
		break;
	}
	ret = 0;
...
static long akm09916_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_ECS_IOCTL_WRITE:
		MAGN_LOG("COMPAT_ECS_IOCTL_WRITE\n");
		if (arg32 == NULL) {
			MAG_ERR("invalid argument.\n");
			return -EINVAL;
		}
		...
		if (ret) {
			MAG_ERR("ECS_IOCTL_WRITE unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_RESET:
		MAGN_LOG("COMPAT_ECS_IOCTL_RESET\n");
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_RESET, (unsigned long)arg32);
		if (ret) {
			MAG_ERR("ECS_IOCTL_RESET unlocked_ioctl failed.\n");
			return ret;
		}
		break;

	case COMPAT_ECS_IOCTL_READ:
		MAGN_LOG("COMPAT_ECS_IOCTL_READ\n");
		if (arg32 == NULL) {
			MAG_ERR("invalid argument.\n");
			return -EINVAL;
		}
	...
	case COMPAT_ECS_IOCTL_GET_INFO:
		MAGN_LOG("COMPAT_ECS_IOCTL_GET_INFO\n");
		if (arg32 == NULL) {
			MAG_ERR("invalid argument.\n");
			return -EINVAL;
		}
		...
		if (ret) {
			MAG_ERR("ECS_IOCTL_GET_INFO unlocked_ioctl failed.\n");
			return ret;
		}
		break;

	case COMPAT_ECS_IOCTL_GET_CONF:
		MAGN_LOG("COMPAT_ECS_IOCTL_GET_CONF\n");
		if (arg32 == NULL) {
			MAG_ERR("invalid argument.\n");
			return -EINVAL;
		}
		...
		if (ret) {
			MAG_ERR("ECS_IOCTL_GET_CONF unlocked_ioctl failed.\n");
			return ret;
		}
		break;

	case COMPAT_ECS_IOCTL_SET_MODE:
		MAGN_LOG("COMPAT_ECS_IOCTL_SET_MODE\n");
		if (arg32 == NULL) {
			MAG_ERR("invalid argument.\n");
			return -EINVAL;
		}
		...
		if (ret) {
			MAG_ERR("ECS_IOCTL_SET_MODE unlocked_ioctl failed.\n");
			return ret;
		}
		break;

	case COMPAT_ECS_IOCTL_GETDATA:
		MAGN_LOG("COMPAT_ECS_IOCTL_GETDATA\n");
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GETDATA, (unsigned long)(arg32));
		if (ret) {
			MAG_ERR("ECS_IOCTL_GETDATA unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_SET_YPR_09911:
		MAGN_LOG("COMPAT_ECS_IOCTL_SET_YPR_09911\n");
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_SET_YPR_09911, (unsigned long)(arg32));
		if (ret) {
			MAG_ERR("ECS_IOCTL_SET_YPR_09911 unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_GET_OPEN_STATUS:
		MAGN_LOG("COMPAT_ECS_IOCTL_GET_OPEN_STATUS\n");
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_OPEN_STATUS, (unsigned long)(arg32));
		if (ret) {
			MAG_ERR("ECS_IOCTL_GET_OPEN_STATUS unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_GET_CLOSE_STATUS:
		MAGN_LOG("COMPAT_ECS_IOCTL_GET_CLOSE_STATUS\n");
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_CLOSE_STATUS, (unsigned long)(arg32));
		if (ret) {
			MAG_ERR("ECS_IOCTL_GET_CLOSE_STATUS unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_GET_OSENSOR_STATUS:
		MAGN_LOG("COMPAT_ECS_IOCTL_GET_OSENSOR_STATUS\n");
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_OSENSOR_STATUS, (unsigned long)(arg32));
		if (ret) {
			MAG_ERR("ECS_IOCTL_GET_OSENSOR_STATUS unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_GET_DELAY_09911:
		MAGN_LOG("COMPAT_ECS_IOCTL_GET_DELAY_09911\n");
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_DELAY_09911, (unsigned long)(arg32));
		if (ret) {
			MAG_ERR("ECS_IOCTL_GET_DELAY_09911 unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_GET_LAYOUT_09911:
		MAGN_LOG("COMPAT_ECS_IOCTL_GET_LAYOUT_09911\n");
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_LAYOUT_09911, (unsigned long)arg32);
		if (ret) {
			MAG_ERR("ECS_IOCTL_GET_LAYOUT_09911 unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_MSENSOR_IOCTL_READ_CHIPINFO:
		MAGN_LOG("COMPAT_MSENSOR_IOCTL_READ_CHIPINFO\n");
		ret = file->f_op->unlocked_ioctl(file, MSENSOR_IOCTL_READ_CHIPINFO, (unsigned long)arg32);
		if (ret) {
			MAG_ERR("MSENSOR_IOCTL_READ_CHIPINFO unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_MSENSOR_IOCTL_READ_SENSORDATA:
		MAGN_LOG("COMPAT_MSENSOR_IOCTL_READ_SENSORDATA\n");
		ret = file->f_op->unlocked_ioctl(file, MSENSOR_IOCTL_READ_SENSORDATA, (unsigned long)arg32);
		if (ret) {
			MAG_ERR("MSENSOR_IOCTL_READ_SENSORDATA unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_MSENSOR_IOCTL_SENSOR_ENABLE:
		MAGN_LOG("COMPAT_MSENSOR_IOCTL_SENSOR_ENABLE\n");
		if (arg32 == NULL) {
		...
		if (ret) {
			MAG_ERR("MSENSOR_IOCTL_SENSOR_ENABLE unlocked_ioctl failed.\n");
			return ret;
		}
	...
	case COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
		MAGN_LOG("COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA\n");
		if (arg32 == NULL) {
			MAG_ERR("invalid argument.\n");
			return -EINVAL;
		}
		...
		if (ret) {
			MAG_ERR("MSENSOR_IOCTL_READ_FACTORY_SENSORDATA unlocked_ioctl failed.\n");
			return ret;
		}
		break;
