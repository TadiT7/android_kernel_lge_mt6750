...
static unsigned short EEPROM_Address_BRCB[2] = { 0x0, 0x0 };
...
int iWriteEEPROM_BRCB032GWZ_3(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	int i4RetValue = 0;
	u32 u4Index = 0;
	...
	if (a_u4Bytes + 2 > 8) {
		EEPROMDB
		    ("[BRCB032GWZ_3EEPROM] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
	...
	if (i4RetValue != (a_u4Bytes + 2)) {
		EEPROMDB("[BRCB032GWZ_3EEPROM] I2C write  failed!!\n");
		return -1;
	}
...
int iReadEEPROM_BRCB032GWZ_3(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	int i4RetValue = 0;
	...
	if (ui4_length > 8) {
		EEPROMDB("[BRCB032GWZ_3EEPROM] exceed I2c-mt65xx.c 8 bytes limitation\n");
		return -1;
	}
...
int iWriteData_BRCB032GWZ_3(unsigned int ui4_offset, unsigned int ui4_length,
			    unsigned char *pinputdata)
{
	...
	EEPROMDB("[BRCB032GWZ_3EEPROM] iWriteData\n");

#if 0
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB
		    ("[BRCB032GWZ_3EEPROM] Write Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	EEPROMDB("[BRCB032GWZ_3EEPROM] iWriteData u4CurrentOffset is %d\n", u4CurrentOffset);
...
#endif
	EEPROMDB("[BRCB032GWZ_3EEPROM] iWriteData done\n");
...
int iReadDataFromBRCB032GWZ_3(unsigned int ui4_offset, unsigned int ui4_length,
			      unsigned char *pinputdata)
{
	...
	/* unsigned short EEPROM_Address_BRCB[2] = {0xA0,0xA0} ; */
...
#if 0
	EEPROM_Address_BRCB[0] =
	    ((0 < address_offset) ? (EEPROM_Address_BRCB[0] | (address_offset - 1)) : EEPROM_Address_BRCB[0]);
	EEPROM_Address_BRCB[1] = ((EEPROM_Address_BRCB[0] & 0xF0) | (address_offset));

	EEPROM_Address_BRCB[0] = EEPROM_Address_BRCB[0] << 1;
	EEPROM_Address_BRCB[1] = EEPROM_Address_BRCB[1] << 1;
#endif

	EEPROMDB("[EEPROM] EEPROM_Address_BRCB[0]=%x EEPROM_Address_BRCB[1]=%x\n", (EEPROM_Address_BRCB[0]),
		 (EEPROM_Address_BRCB[1]));
		...
		do {
			if (16 <= loop[loopCount]) {
				...
				EEPROMDB("puSendCmd[0]=%x puSendCmd[1]=%x, EEPROM(%x)\n",
					 puSendCmd[0], puSendCmd[1], EEPROM_Address_BRCB[loopCount]);
				/* iReadRegI2C(puSendCmd , 2, (u8*)pBuff,16,EEPROM_Address_BRCB[loopCount]); */
				i4RetValue =
				    iBurstReadRegI2C(puSendCmd, 2, (u8 *) pBuff, 16,
						     EEPROM_Address_BRCB[loopCount]);
				if (i4RetValue != 0) {
...
int iReadData_BRCB032GWZ_3(unsigned int ui4_offset, unsigned int ui4_length,
			   unsigned char *pinputdata)
{
	int i4RetValue = 0;
	int i4ResidueDataLength;
...
/* EEPROMDB("[S24EEPORM] iReadData\n" ); */
	...
	if (ui4_offset + ui4_length >= 0x2000) {
		EEPROMDB
		    ("[BRCB032GWZ_3EEPROM] Read Error!! S-24CS64A not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			i4RetValue = iReadEEPROM_BRCB032GWZ_3((u16) u4CurrentOffset, 8, pBuff);
			if (i4RetValue != 0) {
				EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
				return -1;
			}
			...
			i4RetValue =
			    iReadEEPROM_BRCB032GWZ_3((u16) u4CurrentOffset, i4ResidueDataLength,
						     pBuff);
			if (i4RetValue != 0) {
				EEPROMDB("[EEPROM] I2C iReadData failed!!\n");
				return -1;
...
int BRCB032GWZ_3_EEPROM_Ioctl(struct file *file, unsigned int a_u4Command, unsigned long a_u4Param)
#else
...
#endif				/* COMMON_CAM_CAL_DRV */
{
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (NULL == pBuff) {
			EEPROMDB("[BRCB032GWZ_3EEPROM] ioctl allocate mem failed\n");
			return -ENOMEM;
		}
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			...
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
				kfree(pBuff);
				EEPROMDB("[BRCB032GWZ_3EEPROM] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	EEPROMDB("[BRCB032GWZ_3EEPROM] init Working buffer address 0x%8x  command is 0x%8x\n",
	...
	case CAM_CALIOC_S_WRITE:
		EEPROMDB("[BRCB032GWZ_3EEPROM] Write CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		/* i4RetValue = iWriteData_BRCB032GWZ_3((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff); */
#ifdef EEPROMGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		EEPROMDB("[BRCB032GWZ_3EEPROM] Read CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		EEPROMDB("[BRCB032GWZ_3EEPROM] offset %x\n", ptempbuf->u4Offset);
		EEPROMDB("[BRCB032GWZ_3EEPROM] length %x\n", ptempbuf->u4Length);
		EEPROMDB("[BRCB032GWZ_3EEPROM] Before read Working buffer address 0x%8x\n",
			 (u32) pWorkingBuff);
		...
		EEPROMDB("[BRCB032GWZ_3EEPROM] After read Working buffer data  0x%4x\n",
			 *pWorkingBuff);
#else
		ResetCheck = 0;
		...
		if (ptempbuf->u4Offset == 0x800)
			*(u16 *) pWorkingBuff = 0x3;
			 ...
			 && (EEPROM_Address_BRCB[0] == 0x0)) {
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			EEPROM_Address_BRCB[0] = (ptempbuf->u4Offset & 0xffff0000) >> 20;
			EEPROM_Address_BRCB[1] = (ptempbuf->u4Offset & 0xffff0000) >> 20;
			EEPROMDB("[BRCB032GWZ_3EEPROM] Init I2C address %x\n",
				 *(u32 *) pWorkingBuff);
		} else if (((ptempbuf->u4Offset & 0x000FFFFF) == 0x000BCB32)
			   && (ResetCheck == 0xABCDFEDC)) {
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			EEPROMDB("[BRCB032GWZ_3EEPROM] Reset I2C address %x\n",
				 *(u32 *) pWorkingBuff);
		} else {
			readTryagain = 3;
			while (0 < readTryagain) {
				i4RetValue =
				    iReadDataFromBRCB032GWZ_3(ptempbuf->u4Offset,
							      ptempbuf->u4Length, pWorkingBuff);
				EEPROMDB("[BRCB032GWZ_3EEPROM] error (%d) Read retry (%d)\n",
		...
		break;
	default:
		EEPROMDB("[BRCB032GWZ_3EEPROM] No CMD\n");
		i4RetValue = -EPERM;
		break;
	}
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		EEPROMDB("[BRCB032GWZ_3EEPROM] to user length %d\n", ptempbuf->u4Length);
		EEPROMDB("[BRCB032GWZ_3EEPROM] to user  Working buffer address 0x%8x\n",
			 (u32) pWorkingBuff);
		if (copy_to_user
		    ((u8 __user *) ptempbuf->pu1Params, (u8 *) pWorkingBuff, ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pWorkingBuff);
			EEPROMDB("[BRCB032GWZ_3EEPROM] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
{
	EEPROMDB("[BRCB032GWZ_3EEPROM] EEPROM_Open\n");
	spin_lock(&g_EEPROMLock);
	if (g_u4Opened) {
		spin_unlock(&g_EEPROMLock);
		return -EBUSY;
...
static int RegisterEEPROMCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_EEPROMdevno, 0, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[BRCB032GWZ_3EEPROM] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_EEPROMdevno, 1, EEPROM_DRVNAME)) {
		EEPROMDB("[BRCB032GWZ_3EEPROM] Register device no failed\n");
	...
	if (NULL == g_pEEPROM_CharDrv) {
		...
		EEPROMDB("[BRCB032GWZ_3EEPROM] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pEEPROM_CharDrv, g_EEPROMdevno, 1)) {
		EEPROMDB("[BRCB032GWZ_3EEPROM] Attatch file operation failed\n");
...
static unsigned short force[] = {
	EEPROM_I2C_GROUP_ID, BRCB032GWZ_3_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END
};
#else
/* static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, BRCB032GWZ_3_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END}; */
#endif
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	EEPROMDB("[BRCB032GWZ_3EEPROM] Attach I2C\n");
/* spin_lock_init(&g_EEPROMLock); */
	...
	if (i4RetValue) {
		EEPROMDB("[BRCB032GWZ_3EEPROM] register char device failed!\n");
		return i4RetValue;
	}


	EEPROMDB("[BRCB032GWZ_3EEPROM] Attached!!\n");
	return 0;
}
...
static int __init EEPROM_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stEEPROM_Driver)) {
		EEPROMDB("failed to register BRCB032GWZ_3EEPROM driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stEEPROM_Device)) {
		EEPROMDB("failed to register BRCB032GWZ_3EEPROM driver, 2nd time\n");
		return -ENODEV;
	}
...
#if defined(COMMON_EEPROM)
EXPORT_SYMBOL(BRCB032GWZ_3_EEPROM_Ioctl);
#else
