			  ...
			  unsigned char *pinputdata)
{
	...
	/* unsigned char address_offset =
		 ((SampleNum *SampleOffset) + EEPROM_Address_Offset+ ui4_length) / Boundary_Address; */
	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				EEPROMDB
				    ("[EEPROM]1 loopCount=%d loop[loopCount]=%d\n",
				     loopCount, loop[loopCount]);
				EEPROMDB
				    ("[EEPROM]1 puSendCmd[0]=%x puSendCmd[1]=%x, EEPROM(%x)\n",
...
#endif				/* COMMON_EEPROM */
{
	...
	if (NULL == pWorkingBuff) {
		kfree(pBuff);
		EEPROMDB("[AT24C16D] ioctl allocate mem failed\n");
		return -ENOMEM;
	}
	EEPROMDB("[AT24C16D] init Working buffer address 0x%8x  command is 0x%8x\n",
		 (u32) pWorkingBuff, (u32) a_u4Command);

	if (copy_from_user((u8 *) pWorkingBuff, (u8 *) ptempbuf->pu1Params, ptempbuf->u4Length)) {
		kfree(pBuff);
		kfree(pWorkingBuff);
		EEPROMDB("[AT24C16D] ioctl copy from user failed\n");
		return -EFAULT;
	}
	...
	case CAM_CALIOC_G_READ:
		EEPROMDB("[AT24C16D] Read CMD\n");
#ifdef EEPROMGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		EEPROMDB("[AT24C16D] offset %x\n", ptempbuf->u4Offset);
		EEPROMDB("[AT24C16D] length %x\n", ptempbuf->u4Length);
		EEPROMDB("[AT24C16D] Before read Working buffer address 0x%8x\n",
			 (u32) pWorkingBuff);
#if 0
		...
		if (ptempbuf->u4Offset == 0x800) {
		...
		} else if (((ptempbuf->u4Offset & 0x000FFFFF) == 0x00024C16)
			...
			*(u32 *) pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
			...
			EEPROMDB("[AT24C16D] Reset I2C address %x\n", *(u32 *) pWorkingBuff);
		} else {
			readTryagain = 3;
			while (0 < readTryagain) {
				i4RetValue =
				    iReadDataFromAT24C16D(ptempbuf->u4Offset, ptempbuf->u4Length,
							  pWorkingBuff);
				EEPROMDB("[AT24C16D] error (%d) Read retry (%d)\n", i4RetValue,
					 readTryagain);
				if (i4RetValue != 0)
					readTryagain--;
				else
		...
		EEPROMDB("[AT24C16D] After read Working buffer data  0x%4x\n", *pWorkingBuff);
#endif
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		EEPROMDB("[AT24C16D] to user length %d\n", ptempbuf->u4Length);
		EEPROMDB("[AT24C16D] to user  Working buffer address 0x%8x\n", (u32) pWorkingBuff);
		if (copy_to_user
		    ((u8 __user *) ptempbuf->pu1Params, (u8 *) pWorkingBuff, ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pWorkingBuff);
			EEPROMDB("[AT24C16D] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
