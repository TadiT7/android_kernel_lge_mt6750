...
/*
 * ZC524AFWT voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "ZC524AFWT_DRV"
...
static int s4ZC524AF_ReadReg(unsigned short *a_pu2Result)
{
...
static int s4AF_WriteReg(u16 a_u2Data)
{
    ...
    if(status_AF == AF_initializing)
        puSendCmd[1] |= 0x0F;		//4steps & 648us per steps
    else if(status_AF == AF_releasing)
        puSendCmd[1] |= 0x0D;		//4steps & 162us per steps
    else
...
{
    char puSendCmd_ENTRY_1_ON[2] = {0xEC, 0xA3};  	//ENTRY_1_ON
    char puSendCmd1[2] = {0xA1, 0x05};					//TSC mode OFF & TDIV = X2
    char puSendCmd2[2] = {0xF2, 0x80};					//SC = 0b10000
    char puSendCmd_ENTRY_1_OFF[2] = {0xDC, 0x51}; 	//ENTRY_1_OFF
    char puSendCmd_ENTRY_2_ON[2] = {0xEC, 0xAB};  	//ENTRY_2_ON
    char puSendCmd3[2] = {0x80, 0x09};					//DSC_MODE(DSC1)
    char puSendCmd4[2] = {0x81, 0xC8};					//VCM_DAMP_PERIOD(200 x 0.1ms) = 20ms
    char puSendCmd5[2] = {0x82, 0x64};                  //initial position 0x64(100)x2 = 200
    char puSendCmd6[2] = {0x83, 0x03};					//AIP_TIME(500us)
    char puSendCmd_ENTRY_2_OFF[2] = {0xDC, 0x59}; 	//ENTRY_2_OFF
    ...
    i2c_master_send(g_pstAF_I2Cclient, puSendCmd_ENTRY_2_ON, 2); mdelay(1);
    ...
    i2c_master_send(g_pstAF_I2Cclient, puSendCmd6, 2); mdelay(1);
    i2c_master_send(g_pstAF_I2Cclient, puSendCmd_ENTRY_2_OFF, 2); mdelay(30);
}

static void set_reg_drive(void)
{
    char puSendCmd_ENTRY_1_ON[2] = {0xEC, 0xA3};  	//ENTRY_1_ON
    char puSendCmd1[2] = {0xA1, 0x05};					//TSC mode Off & TDIV = X1(default)
    char puSendCmd2[2] = {0xF2, 0x00};					//SC = 0b00000
    char puSendCmd_ENTRY_1_OFF[2] = {0xDC, 0x51}; 	//ENTRY_1_OFF
    char puSendCmd_ENTRY_2_ON[2] = {0xEC, 0xAB};  	//ENTRY_2_ON
    char puSendCmd3[2] = {0x80, 0x09};					//DSC_MODE(DSC1)
    char puSendCmd4[2] = {0x81, 0x64};					//VCM_DAMP_PERIOD(100 x 0.1ms) = 10ms
    char puSendCmd_ENTRY_2_OFF[2] = {0xDC, 0x59}; 	//ENTRY_2_OFF
    ...
    i2c_master_send(g_pstAF_I2Cclient, puSendCmd3, 2); mdelay(1);
    i2c_master_send(g_pstAF_I2Cclient, puSendCmd4, 2); mdelay(1);
    i2c_master_send(g_pstAF_I2Cclient, puSendCmd_ENTRY_2_OFF, 2); mdelay(1);
}
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	//LOG_INF("Init Pos ,g_u4CurrPosition %ld,g_u4TargetPosition:%ld\n",g_u4CurrPosition,g_u4TargetPosition);
    ...
    {
        LOG_INF("[ZC524AF] set_initializing_mode\n");
        set_reg_init();
...
/* ////////////////////////////////////////////////////////////// */
long ZC524AFWT_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
    long i4RetValue = 0;
...
int ZC524AFWT_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
    LOG_INF("Start\n");
...
void ZC524AFWT_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
