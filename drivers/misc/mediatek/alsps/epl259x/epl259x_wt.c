...
#define DRIVER_VERSION          "3.0.4_lsrc"
/******************************************************************************
...
static int PS_h_offset = 5093;  //factory high threshold offset
static int PS_l_offset = 2029;  //factory low threshold offset
...
static void write_global_variable(struct i2c_client *client)
{
	...
	if(enable_hs)
	{
	    ...
	    buf_block[0] = epl_sensor.hs.integration_time | epl_sensor.hs.gain; //REG0x03
        buf_block[1] = epl_sensor.hs.adc | epl_sensor.hs.cycle;
        buf_block[2] = epl_sensor.hs.ir_on_control | epl_sensor.hs.ir_mode | epl_sensor.hs.ir_driver;
        buf_block[3] = epl_sensor.interrupt_control | epl_sensor.ps.persist |epl_sensor.ps.interrupt_type;
        buf_block[4] =epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type;
        epl_sensor_I2C_Write_Block(client, 0x03, buf_block, 5)

		epl_sensor_I2C_Write(client, 0x1b, epl_sensor.hs.compare_reset | epl_sensor.hs.lock);
	}
#if !PS_GES	/*PS_GES*/
	else
#elif PS_GES
...
#endif  /*HS_ENABLE*/
    {
	    ...
	    /*ges setting*/
        buf_block[0] = epl_sensor.ges.integration_time | epl_sensor.ges.gain; //REG0x03
        buf_block[1] = epl_sensor.ges.adc | epl_sensor.ges.cycle;
        buf_block[2] = epl_sensor.ges.ir_on_control | epl_sensor.ges.ir_mode | epl_sensor.ges.ir_drive;
        buf_block[3] = epl_sensor.interrupt_control | epl_sensor.ps.persist |epl_sensor.ps.interrupt_type;
        buf_block[4] = epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type;
        epl_sensor_I2C_Write_Block(client, 0x03, buf_block, 5);

        epl_sensor_I2C_Write(client, 0x1b, (epl_sensor.ges.compare_reset | epl_sensor.ges.lock));
    }
    else
#endif /*PS_GES*/
    {
        ...
        if(epl_sensor.revno != 0x8188)
        {
            if(epl_sensor.revno == 0x0288)
            {
                ...
                buf_block[1] = epl_sensor.als.als_rs | epl_sensor.als.adc | epl_sensor.als.cycle;
                buf_block[2] = epl_sensor.ps.ps_std | epl_sensor.ps.integration_time | epl_sensor.ps.gain;
                buf_block[3] = epl_sensor.ps.ps_rs | epl_sensor.ps.adc | epl_sensor.ps.cycle;
        ...
        else
        {
            ...
            buf_block[1] = epl_sensor.als.adc | epl_sensor.als.cycle;
            buf_block[2] = epl_sensor.ps.integration_time | epl_sensor.ps.gain;
            buf_block[3] = epl_sensor.ps.adc | epl_sensor.ps.cycle;
            buf_block[4] = epl_sensor.ps.ir_on_control | epl_sensor.ps.ir_mode | epl_sensor.ps.ir_drive;
            buf_block[5] = epl_sensor.interrupt_control | epl_sensor.ps.persist |epl_sensor.ps.interrupt_type;
            buf_block[6] = epl_sensor.als.interrupt_channel_select | epl_sensor.als.persist | epl_sensor.als.interrupt_type;

        }
        epl_sensor_I2C_Write_Block(client, 0x01, buf_block, 7);
#if !ALS_DYN_INTT
    ...
    //set mode and wait
    if(epl_sensor.revno == 0x0288)
        epl_sensor_I2C_Write(client, 0x00, (epl_sensor.wait | epl_sensor.als_single_pixel | epl_sensor.mode));
    else
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.wait = EPL_WAIT_100_MS;
    ...
    c_gain = 245; // 245/1000=0.245 /*Lux per count*/
    ...
    switch(epl_sensor.als.report_type)
    {
		...
		case CMC_BIT_DYN_INT:
            ...
            if(change_flag == true)
            {
                ...
                if(epl_sensor.revno == 0x0288)
                    epl_sensor_I2C_Write(obj->client, 0x00, epl_sensor.wait | epl_sensor.als_single_pixel | epl_sensor.mode);
                else
...
//#endif
    ...
    APS_LOG("[%s]: IDEL MODE \r\n", __func__);
    if(epl_sensor.revno == 0x0288)
        epl_sensor_I2C_Write(obj->client, 0x00, epl_sensor.wait | epl_sensor.als_single_pixel |  EPL_MODE_IDLE);
    else
...
int epl_sensor_check_reg(struct i2c_client *client)
{
    u8 buf[2];
    mutex_lock(&sensor_mutex);
    epl_sensor_I2C_Read(client,0x05, 2);
    buf[0] = gRawData.raw_bytes[0];
    buf[1] = gRawData.raw_bytes[1];
    mutex_unlock(&sensor_mutex);
        ...
        || buf[1] != (epl_sensor.interrupt_control | epl_sensor.ps.persist |epl_sensor.ps.interrupt_type))
    {
        ...
        epl_sensor_I2C_Write(client,0x1b, EPL_CMP_RESET | EPL_UN_LOCK);
        epl_sensor_I2C_Write(client,0x1b, EPL_CMP_RUN | EPL_UN_LOCK);
        mutex_unlock(&sensor_mutex);
        return -1;
    }
    return 0;
...
void epl_sensor_fast_update(struct i2c_client *client)
{
    ...
    if(epl_sensor.revno == 0x0288)
    {
        epl_sensor_I2C_Write(client, 0x01, epl_sensor.als.als_std | epl_sensor.als.integration_time | epl_sensor.als.gain);
        epl_sensor_I2C_Write(client, 0x00, epl_sensor.wait | epl_sensor.als_single_pixel | EPL_MODE_ALS);
    }
    else
    {
...
static ssize_t epl_sensor_store_integration(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    switch (epl_sensor.mode)
    {
        case EPL_MODE_PS:
            ...
            if(epl_sensor.revno == 0x0288)
                epl_sensor_I2C_Write(obj->client, 0x03, epl_sensor.ps.ps_std | epl_sensor.ps.integration_time | epl_sensor.ps.gain);
            else
        ...
        case EPL_MODE_ALS: //als
...
#endif
            {
                ...
                if(epl_sensor.revno == 0x0288)
                    epl_sensor_I2C_Write(obj->client, 0x01, epl_sensor.als.als_std | epl_sensor.als.integration_time | epl_sensor.als.gain);
                else
...
static ssize_t epl_sensor_store_gain(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	switch (epl_sensor.mode)
	{
        case EPL_MODE_PS:
            ...
            if(epl_sensor.revno == 0x0288)
                epl_sensor_I2C_Write(epld->client, 0x03, epl_sensor.ps.ps_std | epl_sensor.ps.integration_time | epl_sensor.ps.gain);
            else
        ...
        case EPL_MODE_ALS: //als
            ...
            if(epl_sensor.revno == 0x0288)
                epl_sensor_I2C_Write(epld->client, 0x01, epl_sensor.als.als_std | epl_sensor.als.integration_time | epl_sensor.als.gain);
            else
...
static int epl_sensor_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    alsps_init_flag = 0;
#endif
    //hardwareinfo_set_prop(HARDWARE_ALSPS,"MN26233TKDN");//bug314053,gaojingxuan.wt,add hardwareinfo for factory mode
    printk("ALSPS IIC probe OK\n");
    return 0;
