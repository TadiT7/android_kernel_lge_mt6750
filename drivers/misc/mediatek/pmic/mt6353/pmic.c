...
void force_pwm_set(void)
{
	int ret = 0;

	pr_info("force_pwm_set\n");

	/* force PWM for VS1 VS1_ANA_CON1 0x0462 */
	ret = pmic_config_interface(PMIC_RG_VS1_MODESET_ADDR, 1, PMIC_RG_VS1_MODESET_MASK, PMIC_RG_VS1_MODESET_SHIFT);
	/* force PWM for VPA VPA_ANA_CON1 0x0442 */
	ret = pmic_config_interface(PMIC_RG_VPA_MODESET_ADDR, 1, PMIC_RG_VPA_MODESET_MASK, PMIC_RG_VPA_MODESET_SHIFT);
	/* force PWM for VCORE1 VCORE_ANA_CON1 0x044A */
	ret = pmic_config_interface(PMIC_RG_VCORE_MODESET_ADDR, 1, PMIC_RG_VCORE_MODESET_MASK, PMIC_RG_VCORE_MODESET_SHIFT);
	/* force PWM for VCORE2 VCORE2_ANA_CON1 0x046C*/
	ret = pmic_config_interface(PMIC_RG_VCORE2_MODESET_ADDR, 1, PMIC_RG_VCORE2_MODESET_MASK, PMIC_RG_VCORE2_MODESET_SHIFT);
	/* force PWM for VPROC VPROC_ANA_CON1 0x0454 */
	ret = pmic_config_interface(PMIC_RG_VPROC_MODESET_ADDR, 1, PMIC_RG_VPROC_MODESET_MASK, PMIC_RG_VPROC_MODESET_SHIFT);
}
...
void auto_mode_set(void)
{
	...
	pr_info("auto_mode_set\n");

	/* force PWM for VS1 VS1_ANA_CON1 0x0462 */
	ret = pmic_config_interface(PMIC_RG_VS1_MODESET_ADDR, 0, PMIC_RG_VS1_MODESET_MASK, PMIC_RG_VS1_MODESET_SHIFT);
	/* force PWM for VPA VPA_ANA_CON1 0x0442 */
	ret = pmic_config_interface(PMIC_RG_VPA_MODESET_ADDR, 0, PMIC_RG_VPA_MODESET_MASK, PMIC_RG_VPA_MODESET_SHIFT);
	/* force PWM for VCORE1 VCORE_ANA_CON1 0x044A */
	ret = pmic_config_interface(PMIC_RG_VCORE_MODESET_ADDR, 0, PMIC_RG_VCORE_MODESET_MASK, PMIC_RG_VCORE_MODESET_SHIFT);
	/* force PWM for VCORE2 VCORE2_ANA_CON1 0x046C*/
	ret = pmic_config_interface(PMIC_RG_VCORE2_MODESET_ADDR, 0, PMIC_RG_VCORE2_MODESET_MASK, PMIC_RG_VCORE2_MODESET_SHIFT);
	/* force PWM for VPROC VPROC_ANA_CON1 0x0454 */
	ret = pmic_config_interface(PMIC_RG_VPROC_MODESET_ADDR, 0, PMIC_RG_VPROC_MODESET_MASK, PMIC_RG_VPROC_MODESET_SHIFT);
}

static int pmic_pwm = 0;
static int set_pmic_pwm(const char *val, struct kernel_param *kp)
{
	int ret;

	ret = param_set_int(val, kp);
	if (ret) {
		pr_info("%s error setting value %d\n", __func__, ret);
		return ret;
	}
	pr_info("%s: setting value %d\n", __func__, pmic_pwm);
	...
	pr_info("PMIC_REG VS1=0x%x VPA=0x%x VCORE=0x%x VCORE2=0x%x VPROC=0x%x\n",
			upmu_get_reg_value(0x0462), upmu_get_reg_value(0x0442), upmu_get_reg_value(0x044A), upmu_get_reg_value(0x046C), upmu_get_reg_value(0x0454));

	return 0;
}
module_param_call(pmic_pwm, set_pmic_pwm,
		param_get_int, &pmic_pwm, 0644);
