...
static ssize_t mt_log_write(struct file *filp, const char *ubuf, size_t cnt, loff_t *data)
{
	...
	int level = 0;
	unsigned long long computed_second;
	...
	if ((second < 0) || (second > ULLONG_MAX)) {
		SEQ_printf(NULL, "overflow!! please insert second to proper value range(ULL)\n");
		return cnt;
	}

	computed_second = second * 1000000000;
	if (computed_second >= ULLONG_MAX)
		computed_second = ULLONG_MAX;

	for (;;) {
		t2 = sched_clock();
		if (t2 - t1 > computed_second)
			break;
