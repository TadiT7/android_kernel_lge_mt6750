...
//#include <linux/hardware_info.h>//bug314053,gaojingxuan.wt,add hardwareinfo for factory mode 
#include "upmu_sw.h"
...
#define CAL_FAIL		1
#define CAL_FLAT_FAIL	2
#define KIONIX_SHAKING_DETECT_THRESHOLD 72
#define TESTLIMIT_XY					92     //+-179mg/(1g/512) = 91.8
#define TESTLIMIT_Z_LSL					380    //-258mg/(1g/512) = 132.3
#define TESTLIMIT_Z_USL					644    //+258mg/(1g/512) = 132.3
...
static struct data_resolution kxtj2_1009_data_resolution[1] = {
 /* combination by {FULL_RES,RANGE}*/
    {{ 0, 9}, 512}, // dataformat +/-2g  in 12-bit resolution;  { 3, 9} = 3.9 = (2*2*1000)/(2^12);  256 = (2^12)/(2*2)
};
/*----------------------------------------------------------------------------*/
...
static int kxtj2_1009_init_client(struct i2c_client *client, int reset_cali)
{
	...
	if(res != KXTJ2_1009_SUCCESS ) //0x04->BW=100Hz
	{
		return res;
	}

	res = KXTJ2_1009_SetDataFormat(client, KXTJ2_1009_RANGE_4G);
	if(res != KXTJ2_1009_SUCCESS) //0x08->RANGE_4G
	{
		return res;
	}
...
/*Kionix Auto-Cali End*/
		/*adjust calibration*/
		obj->data[KXTJ2_1009_AXIS_X] += obj->accel_cali[KXTJ2_1009_AXIS_X];
		obj->data[KXTJ2_1009_AXIS_Y] += obj->accel_cali[KXTJ2_1009_AXIS_Y];
		obj->data[KXTJ2_1009_AXIS_Z] += obj->accel_cali[KXTJ2_1009_AXIS_Z];
...
static int kionix_do_calibration(struct i2c_client *client, int apply_now)
{
	...
	int i;
	int xyz[3] = { 0 };
	int accel_cal_pre[3] = { 0, };
	int err;

	GSE_LOG("%s: apply_now = %d\n", __func__, apply_now);

	err = kxtj2_1009_set_delay(20000000);       //set ODR to 50Hz/20ms
	if (err < 0)
	{
		GSE_ERR("%s: set ODR error, err = %d. Abort.\n", __func__, err);
		return err;
	}

	err = KXTJ2_1009_SetPowerMode(client, true);                          //enable sensor
	if (err < 0)
	{
		GSE_ERR("%s: sensor enable error, err = %d. Abort.\n", __func__, err);
		return err;
	}
	...
	atomic_set(&obj->fast_calib_rslt, CAL_FAIL);

	KXTJ2_1009_ReadData(client, obj->accel_data_raw);
	msleep(100);     							//throw away the first sensor data

	//acceld->accel_cali[0] = acceld->accel_cali[1] = acceld->accel_cali[2] = 0;
	KXTJ2_1009_ReadData(client, obj->accel_data_raw);

	accel_cal_pre[KXTJ2_1009_AXIS_X] = obj->accel_data_raw[KXTJ2_1009_AXIS_X];
	accel_cal_pre[KXTJ2_1009_AXIS_Y] = obj->accel_data_raw[KXTJ2_1009_AXIS_Y];
	accel_cal_pre[KXTJ2_1009_AXIS_Z] = obj->accel_data_raw[KXTJ2_1009_AXIS_Z];
	...
	for(i = 0; i<CAL_COUNT; i++)		//Get multi-group data for average
	{
		KXTJ2_1009_ReadData(client, obj->accel_data_raw);
		GSE_LOG("%s: ==================moved================= \n", __func__);
		GSE_LOG("%s: (%d, %d, %d),(%d, %d, %d) \n", __func__, accel_cal_pre[KXTJ2_1009_AXIS_X],accel_cal_pre[KXTJ2_1009_AXIS_Y],accel_cal_pre[KXTJ2_1009_AXIS_Z]
									,obj->accel_data_raw[KXTJ2_1009_AXIS_X],obj->accel_data_raw[KXTJ2_1009_AXIS_Y],obj->accel_data_raw[KXTJ2_1009_AXIS_Z]);

		if( abs(obj->accel_data_raw[KXTJ2_1009_AXIS_X] - accel_cal_pre[KXTJ2_1009_AXIS_X]) > KIONIX_SHAKING_DETECT_THRESHOLD
			|| abs( obj->accel_data_raw[KXTJ2_1009_AXIS_Y] - accel_cal_pre[KXTJ2_1009_AXIS_Y] ) > KIONIX_SHAKING_DETECT_THRESHOLD
			|| abs( obj->accel_data_raw[KXTJ2_1009_AXIS_Z] - accel_cal_pre[KXTJ2_1009_AXIS_Z] ) > KIONIX_SHAKING_DETECT_THRESHOLD){
		...
		sum_x  += obj->accel_data_raw[KXTJ2_1009_AXIS_X];
		sum_y  += obj->accel_data_raw[KXTJ2_1009_AXIS_Y];
		sum_z  += obj->accel_data_raw[KXTJ2_1009_AXIS_Z];
		GSE_ERR("%s: [%d] raw_x=%d, raw_y=%d, raw_z==%d\n", __func__,
				i,
				obj->accel_data_raw[KXTJ2_1009_AXIS_X],
				obj->accel_data_raw[KXTJ2_1009_AXIS_Y],
				obj->accel_data_raw[KXTJ2_1009_AXIS_Z]);
		msleep(25);
	}
	...
	GSE_ERR("%s: sum_x=%d, sum_y=%d, sum_z=%d\n", __func__,
			sum_x,
			sum_y,
			sum_z);
	//Zero-g offset Check
	if(abs(sum_x) > TESTLIMIT_XY || abs(sum_y) > TESTLIMIT_XY || abs(sum_z) > TESTLIMIT_Z_USL || abs(sum_z) < TESTLIMIT_Z_LSL){
		return CAL_FLAT_FAIL;
	}

	//X, Y, Z offset
	obj->accel_cali[KXTJ2_1009_AXIS_X] = -(sum_x);
	obj->accel_cali[KXTJ2_1009_AXIS_Y] = -(sum_y);
	obj->accel_cali[KXTJ2_1009_AXIS_Z] = -(sum_z + 512);
	GSE_ERR("%s: cali_x=%d, cali_y=%d, cali_z=%d\n", __func__,
			obj->accel_cali[KXTJ2_1009_AXIS_X],
			obj->accel_cali[KXTJ2_1009_AXIS_Y],
			obj->accel_cali[KXTJ2_1009_AXIS_Z]);

	xyz[0] = obj->accel_cali[KXTJ2_1009_AXIS_X];
	xyz[1] = obj->accel_cali[KXTJ2_1009_AXIS_Y];
	xyz[2] = obj->accel_cali[KXTJ2_1009_AXIS_Z];

	sensor_calibration_save(ID_ACCELEROMETER, xyz);
	atomic_set(&obj->fast_calib_rslt, CAL_SUCCESS);
...
static ssize_t kionix_accel_show_run_cali(struct device_driver *ddri, char *buf)
{
	//struct i2c_client *client = kxtj2_1009_i2c_client;
	struct kxtj2_1009_i2c_data *obj = obj_i2c_data;
...
static ssize_t kionix_accel_store_run_cali(struct device_driver *ddri, const char *buf, size_t count)
{
	struct kxtj2_1009_i2c_data *obj = obj_i2c_data;
	...
	ret = kionix_do_calibration(obj->client, 0);
	atomic_set(&obj->fast_calib_rslt, ret);
...
static DRIVER_ATTR(run_fast_calibration, S_IRUGO | S_IWUSR, kionix_accel_show_run_cali, kionix_accel_store_run_cali);
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			if(err!=0){
				...
				obj->accel_cali[KXTJ2_1009_AXIS_X] = cali[KXTJ2_1009_AXIS_X];
				obj->accel_cali[KXTJ2_1009_AXIS_Y] = cali[KXTJ2_1009_AXIS_Y];
				obj->accel_cali[KXTJ2_1009_AXIS_Z] = cali[KXTJ2_1009_AXIS_Z];
			}
#else
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//hardwareinfo_set_prop(HARDWARE_ACCELEROMETER,"KXTJ1058");//bug314053,gaojingxuan.wt,add hardwareinfo for factory mode
