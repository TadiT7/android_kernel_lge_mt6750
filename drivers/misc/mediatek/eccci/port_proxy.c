...
#if defined(CONFIG_LGE_HANDLE_PANIC)
static int modem_crash_dump_state = 0;
int get_modem_crash_dump_state(void)
{
	return modem_crash_dump_state;
}
EXPORT_SYMBOL(get_modem_crash_dump_state);

void set_modem_crash_dump_state(int flag)
{
	modem_crash_dump_state = flag;
}
#endif
...
void drv_tri_panic_by_lvl(int md_id)
{
	...
	if (lge_get_crash_handle_status() &&
		...
		ccci_md_log_level = MD_LOG_LEVEL_MID;
	} else {
		ccci_md_log_level = MD_LOG_LEVEL_LOW;
	}
#endif
