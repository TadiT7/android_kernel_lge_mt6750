...
unsigned char g_lge_mcc[5] = "";
unsigned char g_lge_mnc[5] = "";
unsigned char g_lge_subset[5] = "";
...
void lge_convert_ntcode_format_to_string (struct FactoryNetworkCodeTag *mtk_atcmd_ntcode, char *mcc, char *mnc, char *subset)
{
	int length = 0;
	char buffer[4];

	struct FactoryNetworkCodeTag *in;
...
int ccci_md_prepare_runtime_data(struct ccci_modem *md, struct sk_buff *skb)
{
	...
	struct FactoryNetworkCodeTag *temp_buf = NULL;
		...
		if (rt_feature.support_info.support_mask == CCCI_FEATURE_MUST_SUPPORT) {
			...
			case MISC_INFO_NTCODE:
				...
				else
				{
					...
					printk("jjm_debug : md_cd_send_runtime_data : runtime.feature_4_val[3] = %s\n", (char *)(&rt_f_element.feature[0]));

					kfree(temp_buf_svn);
				}

				temp_buf = (struct FactoryNetworkCodeTag *)kmalloc(EMMC_BLOCK_SIZE, GFP_KERNEL);
