...
int turn_off_md_bootup_timer_monitor(struct ccci_modem *md)
{
	void __iomem *slv_ssta1_reg = ioremap_nocache(0x260702A4, 0x10);
	void __iomem *slv_ssta_cfg_reg = ioremap_nocache(0x26070608, 0x10);
	unsigned int slv_ssta1, slv5_ssta, slv6_ssta, slv7_ssta;

	cldma_write32(slv_ssta_cfg_reg, 0, 0x135F0000);
	slv_ssta1 = cldma_read32(slv_ssta1_reg, 0);
	slv5_ssta = ((slv_ssta1 & 0xFF00) >> 8);
	slv6_ssta = ((slv_ssta1 & 0xFF0000) >> 16);
	slv7_ssta = ((slv_ssta1 & 0xFF000000) >> 24);

	CCCI_BOOTUP_LOG(md->index, TAG, "slv_ssta1_reg value: 0x%08X\n", slv_ssta1);
	if ((slv5_ssta != 0x1) && (slv5_ssta != 0x2) && (slv5_ssta != 0x4) && (slv5_ssta != 0x8) &&
		(slv5_ssta != 0x10) && (slv5_ssta != 0x20) && (slv5_ssta != 0x40)) {
		CCCI_BOOTUP_LOG(md->index, TAG, "slv5_ssta:0x%08X\n", slv5_ssta);
		goto _err_exit;
	}

	if ((slv6_ssta != 0x1) && (slv6_ssta != 0x2) && (slv6_ssta != 0x4) && (slv6_ssta != 0x8) &&
		(slv6_ssta != 0x10) && (slv6_ssta != 0x20) && (slv6_ssta != 0x40)) {
		CCCI_BOOTUP_LOG(md->index, TAG, "slv6_ssta:0x%08X\n", slv6_ssta);
		goto _err_exit;
	}

	if ((slv7_ssta != 0x1) && (slv7_ssta != 0x2) && (slv7_ssta != 0x4) && (slv7_ssta != 0x8) &&
		(slv7_ssta != 0x10) && (slv7_ssta != 0x20) && (slv7_ssta != 0x40)) {
		CCCI_BOOTUP_LOG(md->index, TAG, "slv7_ssta:0x%08X\n", slv7_ssta);
		goto _err_exit;
	}

	iounmap(slv_ssta1_reg);
	iounmap(slv_ssta_cfg_reg);
...
retry:
	...
	if (turn_off_md_bootup_timer_monitor(md) < 0) {
		retry_count++;
		CCCI_BOOTUP_LOG(md->index, TAG, "slvx_ssta retry: %d\n", retry_count);
		goto retry;
	}
