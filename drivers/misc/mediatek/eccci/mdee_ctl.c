...
void mdee_monitor2_func(struct md_ee *mdee)
{
	...
	*/
	...
	md = ccci_md_get_modem_by_id(mdee->md_id);
	if(md != NULL)
		md->ee_on_going_for_force_off = 0;
	if (md && md->ccci_drv_trigger_upload) {
		pr_err("[md1] drv trigger panic\n");
		drv_tri_panic_by_lvl(mdee->md_id);
		return;
	}

	if (!mdee->is_normal_ee_case) {
		mdee->is_normal_ee_case = 0;
