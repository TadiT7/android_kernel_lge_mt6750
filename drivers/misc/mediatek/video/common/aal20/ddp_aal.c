...
static volatile int g_aal_wait_hist_frame_count;
	...
	if (intsta & 0x2) {	/* End of frame */
		if (spin_trylock_irqsave(&g_aal_hist_lock, flags)) {
			...
			for (i = 0; i < AAL_HIST_BIN; i++) {
				...
				if(g_aal_hist.maxHist[i] != g_aal_hist_db.maxHist[i])
					hist_check++;
			}
			...
			if (hist_check == 0 && g_aal_wait_hist_frame_count > 0)
				g_aal_wait_hist_frame_count--;
			if (g_aal_wait_hist_frame_count == 0)
				g_aal_dirty_frame_retrieved = 1;
