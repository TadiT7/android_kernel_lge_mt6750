...
{
	if (gSetShutdown != STATUS_ON && display_status == STATUS_ON) {
	    LG_ESD_recovery();
	} else {
...
#endif
	/* Notify listeners */
	sysfs_notify(&fbdev->fb_info->dev->kobj, NULL, "show_blank_event");
	return 0;
}
#endif
...
static ssize_t comfort_view_show(struct device *device, struct device_attribute *attr,
        char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%d\n", primary_display_get_comfort_view());
}

static ssize_t comfort_view_store(struct device *device,
        struct device_attribute *attr,
        const char *buf, size_t count)
{
    char **last = NULL;
...
static ssize_t gamma_cmd_show(struct device *device, struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "set gamma %s\n", primary_display_get_surface_temp()? "enable":"disable");
}

static ssize_t  gamma_cmd_store(struct device *device,struct device_attribute *attr,const char *buf, size_t count)
{
    char **last = NULL;
    unsigned int set_value = 0;
    ...
    primary_display_set_surface_temp(set_value);

    return count;
}

static DEVICE_ATTR(gamma_cmd, S_IRUGO|S_IWUSR, gamma_cmd_show, gamma_cmd_store);

static ssize_t mtkts_bts_set_debug_log_show(struct device *device, struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "bts debug log %s\n", primary_display_mtkts_bts_get_debug_log()? "enable":"disable");
}

static ssize_t  mtkts_bts_set_debug_log_store(struct device *device,struct device_attribute *attr,const char *buf, size_t count)
{
    char **last = NULL;
    ...
    primary_display_mtkts_bts_set_debug_log(enable);

    return count;
}

static DEVICE_ATTR(bts_log_enable, S_IRUGO|S_IWUSR, mtkts_bts_set_debug_log_show, mtkts_bts_set_debug_log_store);
#endif
...
int store_esd_recovery_resume(void)
{
	...
	|| defined(CONFIG_TUNING_MIPI_CLOCK) || defined(CONFIG_LGE_COMFORT_VIEW) \
		|| defined(CONFIG_LGE_CHECK_SURFACE_TEMPERATURE)
...
#endif
#ifdef CONFIG_LGE_CHECK_SURFACE_TEMPERATURE
    &dev_attr_gamma_cmd.attr,
    &dev_attr_bts_log_enable.attr,
#endif
    NULL,
};
