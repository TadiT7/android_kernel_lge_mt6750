...
static DEFINE_MUTEX(ft8006m_esd_lock);
#endif
...
#endif
					recovery_done = STATUS_ON;
				} else
					break;

			} while (++try_index < esd_try_cnt);

			if (ret) {
				DISPERR("[ESD]after esd recovery %d times, still fail, disable esd check\n",esd_try_cnt);
				primary_display_esd_check_enable(STATUS_OFF);
#if defined(CONFIG_TCL_INCELL_FT8006M_HD_CV1) || defined(CONFIG_TOVIS_INCELL_TD4100_HD_LV7_SPR) || defined(CONFIG_TOVIS_INCELL_LG4894_HD_CV3)
				LG_ESD_recovery();
#else
				primary_display_esd_recovery();
...
int  esd_recovery_status = false;

void LG_ESD_recovery(void)
{
	mutex_lock(&ft8006m_esd_lock);
...
void primary_display_esd_check_enable(int enable)
{
	if (_need_do_esd_check()) {
		if (enable) {
			esd_check_enable = STATUS_ON;
			atomic_set(&_check_task_wakeup, STATUS_ON);
			wake_up_interruptible(&_check_task_wq);
		} else {
			esd_check_enable = STATUS_OFF;
		}
		DISPMSG("[ESD]%s esd check\n",(enable)? "Enable":"Disable");
	} else
