...
int primary_display_get_surface_temp(void)
{
    return disp_lcm_get_gamma_cmd(pgc->plcm);
}

void primary_display_set_surface_temp(unsigned int enable)
{
    _primary_path_lock(__func__);
    ...
    disp_lcm_set_gamma_cmd(pgc->plcm,enable);

    _primary_path_unlock(__func__);
}

int primary_display_mtkts_bts_get_debug_log(void)
{
	return mtkts_bts_get_debug_log();
}

void primary_display_mtkts_bts_set_debug_log(int enable)
{
	mtkts_bts_set_debug_log(enable);
}
#endif
...
#if 0
	DISPDBG("[POWER]lcm resume_cmd[begin]\n");
	disp_lcm_resume_cmd(pgc->plcm);
	DISPMSG("[POWER]lcm resume_cmd[end]\n");
#endif
