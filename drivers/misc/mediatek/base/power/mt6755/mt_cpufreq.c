...
struct trace_data_info *little_volt;
struct trace_data_info *big_volt;
#endif
...
#ifdef CONFIG_LGE_ETRS
	if (cpu_dvfs_is(p, MT_CPU_DVFS_LITTLE))
		TRACE_WRITE(little_volt, &volt);
	else
		TRACE_WRITE(big_volt, &volt);
#endif
...
#ifdef CONFIG_LGE_ETRS
	INIT_TRACE_DATA(little_volt, FORMAT_TYPE_UINT, 50);
	INIT_TRACE_DATA(big_volt, FORMAT_TYPE_UINT, 50);
#endif
