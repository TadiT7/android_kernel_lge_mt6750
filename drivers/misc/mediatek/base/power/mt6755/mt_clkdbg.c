...
static void mt_dump_clk_state(const char *clkname)
{
...
void mt_clk_dump(void)
{
	int i;
	size_t num;
	static const char * const dbg_clks[] = {
	...
	num = ARRAY_SIZE(dbg_clks);

	pr_warn("\n");
	for (i = 0; i < num; i++)
		mt_dump_clk_state(dbg_clks[i]);
}
#endif
