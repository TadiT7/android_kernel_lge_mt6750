...
#define EXTRA_INDEX_MAX 20
static u32 sram_extra_info[EXTRA_INDEX_MAX];
...
char *vcorefs_get_sram_debug_info(char *p)
{
	...
	if (p) {
		...
		for (i = 0; i < EXTRA_INDEX_MAX; i++)
			p += sprintf(p, "extra[%d] = 0x%x\n", i, vcorefs_get_sram_data(i));
		p += sprintf(p, "\n");
		...
		for (i = 0; i < EXTRA_INDEX_MAX; i++) {
			sram_extra_info[i] = vcorefs_get_sram_data(i);
			vcorefs_info("extra[%d] : 0x%x\n", i, sram_extra_info[i]);
		}
	}
	return p;
}
...
uint32_t get_vcore_dvfs_sram_debug_regs(uint32_t index)
{
	...
	switch (index) {
	case 0:
		value = 8 + EXTRA_INDEX_MAX; /* add VCOREFS_SRAM_DRAM_GATING_CHECK */
	...
	default:
		extra_index = index - 9;
		if (extra_index < EXTRA_INDEX_MAX) {
			value = sram_extra_info[extra_index];
			vcorefs_err("EXTRA_INFO(%d) = 0x%x\n", extra_index, value);
		}
	}
