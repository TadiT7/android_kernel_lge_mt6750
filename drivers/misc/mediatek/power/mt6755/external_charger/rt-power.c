	...
	switch (psp)
	{
		case POWER_SUPPLY_PROP_ONLINE:
			...
			if (psy->type == POWER_SUPPLY_TYPE_MAINS)
			{
				...
				{ pi->ac_online = val->intval; rc = rtpower_set_charger(pi);
				}
			}
			else if (psy->type == POWER_SUPPLY_TYPE_USB)
			{
...
static int __init rt_power_probe(struct platform_device *pdev)
{
...
static int __exit rt_power_remove(struct platform_device *pdev)
{
...
static struct platform_driver rt_power_driver = 
{
	...
	.remove = __exit_p(rt_power_remove),
