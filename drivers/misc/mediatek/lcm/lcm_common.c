...
unsigned int lcm_common_read_by_cmdq_vdo(void *handle, unsigned cmd, unsigned char *buffer,
										unsigned char size, unsigned in flag)
{
	DISPFUNC();

	lcm_util.dsi_dcs_read_cmdq_lcm_reg_v2_2(handle, cmd, buffer, size, flag);
...
LCM_DRIVER lcm_common_drv = {
	...
	.read_by_cmdq_vdo = lcm_common_read_by_cmdq_vdo,
#if defined(R63419_WQHD_TRULY_PHANTOM_2K_CMD_OK)
