...
#include <soc/mediatek/lge/lge_charging_scenario.h>
...
static struct batt_temp_table chg_temp_table[CHG_MAXIDX] = {
	{ INT_MIN,       -6,    CHG_BATTEMP_BL_M5,  "under -5" },	// batt_temp < -4
	{     -5,         41,     CHG_BATTEMP_M4_41,   "-4 ~ 41"  },	// -4 <= batt_temp < 42
	{     42,         45,     CHG_BATTEMP_42_45,   "42 ~ 45"  },	// 42 <= batt_temp <= 45
	{     46,         53,     CHG_BATTEMP_46_53,   "46 ~ 53"  },	// 45 < batt_temp <= 53
	{     54,    INT_MAX,     CHG_BATTEMP_AB_OT,   "over 53"  },	// 53 < batt_temp
};
...
determine_lge_charging_state(enum lge_battemp_states battemp_st, int batt_volt)
{
	...
	case CHG_BATT_NORMAL_STATE:
		...
		} else if (battemp_st >= CHG_BATTEMP_46_53 && battemp_st < CHG_BATTEMP_AB_OT) {
