...
int set_chr_enable_otg(unsigned int enable)
{
	...
	battery_charging_control(CHARGING_CMD_ENABLE_POWER_PATH, &enable);
...
static bool mtk_is_pep_series_avaliable(void)
{
	...
	if (g_temp_input_CC_value <= CHARGE_CURRENT_300_00_MA) {
		battery_log(BAT_LOG_CRTI, "[PE+] Skip : IUSB under 300mA\n");
		return false;
	}

	/* battery charging is too low */
	if (g_temp_CC_value <= CHARGE_CURRENT_300_00_MA) {
		battery_log(BAT_LOG_CRTI, "[PE+] Skip : IBAT under 300mA\n");
		return false;
	}
...
{
	bool pep_series_connect;

	if (!batt_cust_data.mtk_pump_express_plus_support)
		return;

	/* check PEP only if DCP connected */
	if (BMT_status.charger_type != STANDARD_CHARGER)
		return;
	...
	pep_series_connect = mtk_is_pep_series_connect();
	...
	/* for faster update */
	if (mtk_is_pep_series_connect() != pep_series_connect) {
...
static void mtk_select_ichg_aicr(void)
{
	...
	battery_log(BAT_LOG_CRTI,
			"[BATTERY] CC mode current: input=%dmA, charge=%dmA\n",
			g_temp_input_CC_value / 100,
			(g_temp_CC_value + g_temp_parallel_CC_value) / 100);
