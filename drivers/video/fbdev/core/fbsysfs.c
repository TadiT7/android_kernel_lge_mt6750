...
extern char *get_lge_panel_id(void);
#else
extern int lge_get_maker_id(void);
#endif

static ssize_t show_lge_panel_id(struct device *device,
             struct device_attribute *attr, char *buf)
{
#ifdef CONFIG_TCL_INCELL_FT8006M_HD_CV1
    return snprintf(buf, PAGE_SIZE, "%s\n", get_lge_panel_id());
    ...
    return snprintf(buf, PAGE_SIZE, "%d\n", lge_get_maker_id());
#endif
// path : /sys/class/graphics/fb0/lge_panel_id
}
#endif
...
#ifdef CONFIG_LGE_USE_PANEL_DUALIZATION
	__ATTR(lge_panel_id, S_IRUGO, show_lge_panel_id, NULL),
#endif
#ifdef CONFIG_FB_BACKLIGHT
