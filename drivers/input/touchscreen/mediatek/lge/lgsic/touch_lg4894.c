...
int lg4894_xfer_msg(struct device *dev, struct touch_xfer_msg *xfer)
{
	...
	for (i = 0; i < xfer->msg_count; i++) {
		...
		if (rx->size)
			memcpy(rx->buf, rx->data + R_HEADER_SIZE, (rx->size - R_HEADER_SIZE));
	}
...
static int lg4894_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
...
int lg4894_ic_info(struct device *dev)
{
	...
	TOUCH_D(BASE_INFO, "version : v%d.%02d, chip : %d, protocol : %d\n"
		...
		(bootmode >> 2 & 0x1) ? "done" : "booting", (bootmode >> 3 & 0x1) ? "ERROR" : "ok");
...
static int lg4894_get_swipe_data(struct device *dev)
{
	...
	TOUCH_I("Swipe Gesture: start(%4d,%4d) end(%4d,%4d) swipe_time(%dms)\n",
		...
		rdata[1] & 0xffff, rdata[1] >> 16, rdata[2] & 0xffff);
...
static int lg4894_tci_knock(struct device *dev)
{
	...
	return lg4894_reg_write(dev, TCI_ENABLE_W, &lpwg_data[0], sizeof(lpwg_data));
}
...
static int lg4894_tci_active_area(struct device *dev, u32 x1, u32 y1, u32 x2, u32 y2)
{
	int ret = 0, i;
	...
	ret = lg4894_reg_write(dev, ACT_AREA_X1_W, &active_area[0], sizeof(u32));
	ret = lg4894_reg_write(dev, ACT_AREA_Y1_W, &active_area[1], sizeof(u32));
	ret = lg4894_reg_write(dev, ACT_AREA_X2_W, &active_area[2], sizeof(u32));
	ret = lg4894_reg_write(dev, ACT_AREA_Y2_W, &active_area[3], sizeof(u32));
...
static int lg4894_tci_control(struct device *dev, int type)
{
	...
	case ENABLE_CTRL:
		...
		ret = lg4894_reg_write(dev, TCI_ENABLE_W, &lpwg_data, sizeof(lpwg_data));
		break;

	case TAP_COUNT_CTRL:
		lpwg_data = info1->tap_count | (info2->tap_count << 16);
		ret = lg4894_reg_write(dev, TAP_COUNT_W, &lpwg_data, sizeof(lpwg_data));
		break;

	case MIN_INTERTAP_CTRL:
		lpwg_data = info1->min_intertap | (info2->min_intertap << 16);
		ret = lg4894_reg_write(dev, MIN_INTERTAP_W, &lpwg_data, sizeof(lpwg_data));
		break;

	case MAX_INTERTAP_CTRL:
		lpwg_data = info1->max_intertap | (info2->max_intertap << 16);
		ret = lg4894_reg_write(dev, MAX_INTERTAP_W, &lpwg_data, sizeof(lpwg_data));
		break;

	case TOUCH_SLOP_CTRL:
		lpwg_data = info1->touch_slop | (info2->touch_slop << 16);
		ret = lg4894_reg_write(dev, TOUCH_SLOP_W, &lpwg_data, sizeof(lpwg_data));
		break;

	case TAP_DISTANCE_CTRL:
		lpwg_data = info1->tap_distance | (info2->tap_distance << 16);
		ret = lg4894_reg_write(dev, TAP_DISTANCE_W, &lpwg_data, sizeof(lpwg_data));
		break;

	case INTERRUPT_DELAY_CTRL:
		lpwg_data = info1->intr_delay | (info2->intr_delay << 16);
		ret = lg4894_reg_write(dev, INT_DELAY_W, &lpwg_data, sizeof(lpwg_data));
		break;

	case ACTIVE_AREA_CTRL:
		ret = lg4894_tci_active_area(dev,
					     ts->tci.area.x1,
					     ts->tci.area.y1, ts->tci.area.x2, ts->tci.area.y2);
		break;
...
static int lg4894_swipe_active_area(struct device *dev)
{
	...
	ret = lg4894_reg_write(dev, SWIPE_ACT_AREA_X1_W, active_area, sizeof(active_area));
...
static int lg4894_swipe_control(struct device *dev, int type)
{
	...
	case SWIPE_ENABLE_CTRL:
		...
		ret = lg4894_reg_write(dev, SWIPE_ENABLE_W, &swipe_data, sizeof(swipe_data));
		break;
	...
	case SWIPE_DIST_CTRL:
		...
		ret = lg4894_reg_write(dev, SWIPE_DIST_W, &swipe_data, sizeof(swipe_data));
		break;
	case SWIPE_RATIO_THR_CTRL:
		swipe_data = (right->ratio_thres) | (left->ratio_thres << 16);
		ret = lg4894_reg_write(dev, SWIPE_RATIO_THR_W, &swipe_data, sizeof(swipe_data));
		break;
	case SWIPE_RATIO_PERIOD_CTRL:
		swipe_data = (right->ratio_period) | (left->ratio_period << 16);
		ret = lg4894_reg_write(dev, SWIPE_RATIO_PERIOD_W, &swipe_data, sizeof(swipe_data));
		break;
	case SWIPE_RATIO_DIST_CTRL:
		swipe_data = (right->ratio_distance) | (left->ratio_distance << 16);
		ret = lg4894_reg_write(dev, SWIPE_RATIO_DIST_W, &swipe_data, sizeof(swipe_data));
		break;
	case SWIPE_TIME_MIN_CTRL:
		swipe_data = (right->min_time) | (left->min_time << 16);
		ret = lg4894_reg_write(dev, SWIPE_TIME_MIN_W, &swipe_data, sizeof(swipe_data));
		break;
	case SWIPE_TIME_MAX_CTRL:
		swipe_data = (right->max_time) | (left->max_time << 16);
		ret = lg4894_reg_write(dev, SWIPE_TIME_MAX_W, &swipe_data, sizeof(swipe_data));
		break;
...
static int lg4894_swipe_mode(struct device *dev, u8 lcd_mode)
{
	...
	if (lcd_mode != LCD_MODE_U2) {
		...
		ret = lg4894_reg_write(dev, SWIPE_ENABLE_W, &swipe_data[0], sizeof(swipe_data));
...
#ifdef LG4894_TCL_OFF_VIA_MIPI
	struct touch_core_data *ts = to_touch_core(dev);
	/* int status = atomic_read(&ts->state.sleep); [daehyun temp] */
...
*/
/*	primary_display_set_touch_osc(onoff); //0 : osc off / 1 : osc on // [daehyun temp] */
...
static void lg4894_debug_swipe(struct device *dev)
{
	...
	count_max = (count[SWIPE_R] > count[SWIPE_L]) ? count[SWIPE_R] : count[SWIPE_L];
	...
	for (i = 0; i < SWIPE_MAX_NUM; i++) {
		for (j = 0; j < count[i]; j++) {
			...
			TOUCH_I("SWIPE_%s - DBG[%d/%d]: %s\n",
				...
				swipe_debug_str[buf] : swipe_debug_str[0]);
		}
	}
}
...
#ifdef LG4894_TCL_OFF_VIA_MIPI
		if (atomic_read(&d->init) == IC_INIT_NEED) {
			TOUCH_I("Skip lpwg update until IC is initialized!!\n");
			return 0;
		}
#endif
...
#else
	TOUCH_I("%s : Do nothing for charger in 4894\n", __func__);
#endif
}
...
static void lg4894_connect(struct device *dev)
{
	...
	if (charger_state == CONNECT_DISCONNECTED)
		d->charger = CONNECT_NONE;
	else if (charger_state == CONNECT_STANDARD_HOST || charger_state == CONNECT_CHARGING_HOST)
		d->charger = CONNECT_USB;
...
{
	struct lg4894_data *d = container_of(to_delayed_work(fb_notify_work),
...
static int lg4894_notify(struct device *dev, ulong event, void *data)
{
...
#if 0
		ret = lg4894_reg_write(dev, REG_IME_STATE, (u32 *) data, sizeof(u32));
#else
	...
	case NOTIFY_CALL_STATE:
		...
		ret = lg4894_reg_write(dev, REG_CALL_STATE, (u32 *) data, sizeof(u32));
		break;
	default:
...
static int lg4894_fw_upgrade(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
#if 1 /* defined(CONFIG_MACH_MT6755M_K6P) || defined(CONFIG_MACH_MT6750_LGPS32) || defined(CONFIG_MACH_MT6750_LV5) */
...
int lg4894_debug_info(struct device *dev)
{
	...
	if ((atomic_read(&ts->state.earjack) == EARJACK_DEBUG) || (gpio_get_value(126) < 1))
		return ret;
	...
	if (debug_change_mask && press_mask) {
		...
		ret += snprintf(buf + ret, DEBUG_BUF_SIZE - ret,
				...
				debug[id].rn_pos_cnt,
				debug[id].rn_neg_cnt,
				debug[id].rn_pos_sum, debug[id].rn_neg_sum, debug[id].rn_stable);

		for (i = 0; i < ts->tcount; i++) {
			if (i < 1)
				ret += snprintf(buf + ret,
						DEBUG_BUF_SIZE - ret, "[Touch]	tb:");
			ret += snprintf(buf + ret, DEBUG_BUF_SIZE - ret,
		...
		for (i = 0; i < sizeof(debug[id].rn_max_tobj); i++) {
			if (debug[id].rn_max_tobj[i] > 0) {
				if (i < 1)
					ret += snprintf(buf + ret, DEBUG_BUF_SIZE - ret, " to:");
				ret += snprintf(buf + ret,
						DEBUG_BUF_SIZE - ret,
						"%2d ", debug[id].rn_max_tobj[i]);
			} else {
				break;
			}
		}
		...
		for (i = 0; i < sizeof(debug[id].rebase); i++) {
			if (debug[id].rebase[i] > 0) {
				if (i < 1)
					ret += snprintf(buf + ret, DEBUG_BUF_SIZE - ret, " re:");
				ret += snprintf(buf + ret,
						DEBUG_BUF_SIZE - ret, "%2d ", debug[id].rebase[i]);
			} else {
				break;
			}
		}
		...
		for (i = 0; i < sizeof(debug[id].noise_detect); i++) {
			if (debug[id].noise_detect[i] > 0) {
				if (i < 1)
					ret += snprintf(buf + ret, DEBUG_BUF_SIZE - ret, " nd:");
				ret += snprintf(buf + ret,
						DEBUG_BUF_SIZE - ret,
						"%2d ", debug[id].noise_detect[i]);
			} else {
				break;
			}
		}
		...
		for (i = 0; i < sizeof(debug[id].lf_oft); i++) {
			if (debug[id].lf_oft[i] > 0) {
				if (i < 1)
					ret += snprintf(buf + ret, DEBUG_BUF_SIZE - ret, " lf:");
				ret += snprintf(buf + ret,
						DEBUG_BUF_SIZE - ret, "%2x ", debug[id].lf_oft[i]);
			} else {
				break;
			}
		}
		...
		for (i = 0; i < sizeof(debug[id].palm); i++) {
			if (debug[id].palm[i] > 0) {
				if (i < 1)
					ret += snprintf(buf + ret, DEBUG_BUF_SIZE - ret, " pa:");
				ret += snprintf(buf + ret,
						DEBUG_BUF_SIZE - ret, "%2d ", debug[id].palm[i]);
			} else {
				break;
			}
		}
...
int lg4894_irq_lpwg(struct device *dev)
{
	...
	if (d->info.wakeup_type == KNOCK_1) {
		if (ts->lpwg.mode != LPWG_NONE) {
			lg4894_get_tci_data(dev, ts->tci.info[TCI_1].tap_count);
			ts->intr_status = TOUCH_IRQ_KNOCK;
		}
	} else if (d->info.wakeup_type == KNOCK_2) {
		if (ts->lpwg.mode == LPWG_PASSWORD) {
			lg4894_get_tci_data(dev, ts->tci.info[TCI_2].tap_count);
...
int lg4894_irq_handler(struct device *dev)
{
	...
	if (atomic_read(&ts->state.debug_option_mask)
	    & DEBUG_OPTION_2) {
		lg4894_reg_read(dev, tc_ic_status, &d->info, sizeof(d->info));
	} else {
...
static ssize_t show_tci_debug(struct device *dev, char *buf)
{
	...
	if (lg4894_reg_read(dev, TCI_FAIL_DEBUG_R, (u8 *) &rdata, sizeof(rdata)) < 0) {
	...
	ret = snprintf(buf + ret, PAGE_SIZE,
		       ...
		       debug_type[(rdata & 0x8) ? 2 : (rdata & 0x4 ? 1 : 0)]);
...
static ssize_t show_swipe_debug(struct device *dev, char *buf)
{
	...
	if (lg4894_reg_read(dev, SWIPE_FAIL_DEBUG_R, (u8 *) &rdata, sizeof(rdata)) < 0) {
