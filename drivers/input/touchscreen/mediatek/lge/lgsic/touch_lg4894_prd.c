...
static int write_test_control(struct device *dev, u32 mode)
{
	...
	lg4894_reg_read(dev, tc_test_mode_ctl, (u32 *) &test_mode_enter_check, sizeof(u32));
...
static int write_test_mode(struct device *dev, u8 type)
{
	...
	/* TestType Set */
	lg4894_reg_write(dev, tc_tsp_test_ctl, (u8 *) &testmode, sizeof(testmode));
	...
	do {
		touch_msleep(100);
		lg4894_reg_read(dev, tc_tsp_test_sts, (u8 *) &rdata, sizeof(rdata));
		TOUCH_I("rdata = 0x%x\n", rdata);
...
*/
	lg4894_reg_read(dev, prod_open3_short_offset, (u8 *) &os_result_offset, sizeof(u32));
...
static int prd_open_short_test(struct device *dev)
{
	...
	lg4894_reg_read(dev, tc_tsp_test_pf_result, (u8 *) &open_result, sizeof(open_result));
	...
	lg4894_reg_read(dev, tc_tsp_test_pf_result, (u8 *) &short_result, sizeof(short_result));
...
static void prd_read_rawdata(struct device *dev, u8 type)
{
	...
	lg4894_reg_read(dev, m2_raw_data_offset, (u8 *) &raw_offset_info, sizeof(u32));
...
static int prd_compare_rawdata(struct device *dev, u8 type)
{
	...
	switch (type) {
	case DOZE2_M1_RAWDATA_TEST:
		snprintf(lower_str, sizeof(lower_str), "DOZE2_M1_Lower");
		snprintf(upper_str, sizeof(upper_str), "DOZE2_M1_Upper");
		col_size = M1_COL_SIZE + 1;
		m1_raw_buf = M1_Rawdata_buf;
		break;
	case DOZE2_M2_RAWDATA_TEST:
		snprintf(lower_str, sizeof(lower_str), "DOZE2_M2_Lower");
		snprintf(upper_str, sizeof(upper_str), "DOZE2_M2_Upper");
		col_size = COL_SIZE + 1;
		m2_raw_buf = M2_Rawdata_buf;
		break;
	case DOZE1_M2_RAWDATA_TEST:
		snprintf(lower_str, sizeof(lower_str), "DOZE1_M2_Lower");
		snprintf(upper_str, sizeof(upper_str), "DOZE1_M2_Upper");
		col_size = COL_SIZE + 1;
		m2_raw_buf = M2_Rawdata_buf;
		break;
	case DOZE1_BLU_JITTER_TEST:
		snprintf(lower_str, sizeof(lower_str), "DOZE1_BLU_JITTER_Lower");
		snprintf(upper_str, sizeof(upper_str), "DOZE1_BLU_JITTER_Upper");
		col_size = COL_SIZE + 1;
		m2_raw_buf = M2_Rawdata_buf;
		break;
	}
...
if (type == DOZE1_M2_RAWDATA_TEST ||
		   type == DOZE2_M2_RAWDATA_TEST || type == DOZE1_BLU_JITTER_TEST) {
	for (k = 0; k < M1_M2_RAWDATA_TEST_CNT; k++) {
		for (i = 0; i < ROW_SIZE; i++) {
			for (j = 0; j < col_size - 1; j++) {
				if ((m2_raw_buf[k][i * col_size + j] < LowerImage[i][j]) ||
					...
					if ((type != DOZE2_M1_RAWDATA_TEST) &&
					    (i <= 1 && j <= 4) && (m2_raw_buf[k][i * col_size + j] != 0)) {
						result = 1;
						ret +=
						    snprintf(W_Buf + ret,
							     BUF_SIZE - ret,
...
static void tune_display(struct device *dev, char *tc_tune_code, int offset, int type)
{
	...
	switch (type) {
	case 1:
		ret = snprintf(log_buf, tc_tune_code_size, "GOFT tune_code_read : ");
		if ((tc_tune_code[offset] >> 4) == 1) {
			temp[offset] = tc_tune_code[offset] - (0x1 << 4);
			ret += snprintf(log_buf + ret,
					tc_tune_code_size - ret, "-%d  ", temp[offset]);
		} else {
			ret += snprintf(log_buf + ret,
					tc_tune_code_size - ret, " %d  ", tc_tune_code[offset]);
		}
		TOUCH_I("%s\n", log_buf);
		...
		break;
	case 2:
		ret = snprintf(log_buf, tc_tune_code_size, "LOFT tune_code_read : ");
		for (i = 0; i < tc_total_ch_size; i++) {
			if ((tc_tune_code[offset + i]) >> 5 == 1) {
				temp[offset + i] = tc_tune_code[offset + i] - (0x1 << 5);
				ret += snprintf(log_buf + ret,
						tc_tune_code_size - ret, "-%d  ", temp[offset + i]);
			} else {
				ret += snprintf(log_buf + ret,
						tc_tune_code_size - ret,
						" %d  ", tc_tune_code[offset + i]);
			}
		}
		TOUCH_I("%s\n", log_buf);
		ret += snprintf(log_buf + ret, tc_tune_code_size - ret, "\n");
...
static void read_tune_code(struct device *dev, u8 type)
{
	...
	lg4894_reg_read(dev, 0x289, (u8 *) &tune_code_offset, sizeof(u32));
	offset = (tune_code_offset >> 16) & 0xFFFF;

	lg4894_reg_write(dev, tc_tsp_test_data_offset, (u8 *) &offset, sizeof(u32));
	...
	switch (type) {
	case DOZE2_M1_RAWDATA_TEST:
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_L_GOFT_OFFSET, 1);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_R_GOFT_OFFSET, 1);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_L_M1_OFT_OFFSET, 2);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_R_M1_OFT_OFFSET, 2);
		break;
	case DOZE1_M2_RAWDATA_TEST:
	case DOZE2_M2_RAWDATA_TEST:
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_L_GOFT_OFFSET + 1, 1);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_R_GOFT_OFFSET + 1, 1);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_L_G1_OFT_OFFSET, 2);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_L_G2_OFT_OFFSET, 2);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_L_G3_OFT_OFFSET, 2);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_R_G1_OFT_OFFSET, 2);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_R_G2_OFT_OFFSET, 2);
		tune_display(dev, tune_code_read_buf, TSP_TUNE_CODE_R_G3_OFT_OFFSET, 2);
		break;
	}
	write_file(dev, "\n", TIME_INFO_SKIP);
...
static int prd_rawdata_test(struct device *dev, u8 type)
{
	...
	switch (type) {
	case DOZE1_M2_RAWDATA_TEST:
		snprintf(test_type, sizeof(test_type), "[DOZE1_M2_RAWDATA_TEST]");
		break;
	case DOZE2_M1_RAWDATA_TEST:
		snprintf(test_type, sizeof(test_type), "[DOZE2_M1_RAWDATA_TEST]");
		break;
	case DOZE2_M2_RAWDATA_TEST:
		snprintf(test_type, sizeof(test_type), "[DOZE2_M2_RAWDATA_TEST]");
		break;
	case DOZE1_BLU_JITTER_TEST:
		snprintf(test_type, sizeof(test_type), "[DOZE1_BLU_JITTER_TEST]");
		break;
	default:
...
static void ic_run_info_print(struct device *dev)
{
	...
	ret += snprintf(buffer + ret, LOG_BUF_SIZE - ret, "lot : %d\n", rdata[0]);
	ret += snprintf(buffer + ret, LOG_BUF_SIZE - ret, "serial : 0x%X\n", rdata[1]);
	ret += snprintf(buffer + ret, LOG_BUF_SIZE - ret, "date : 0x%X 0x%X\n", rdata[2], rdata[3]);
	ret += snprintf(buffer + ret, LOG_BUF_SIZE - ret,
			...
			(rdata[3] >> 8 & 0xFF), (rdata[3] >> 16 & 0xFF), (rdata[3] >> 24 & 0xFF));
...
static void firmware_version_log(struct device *dev)
{
	...
	ret += snprintf(buffer + ret, LOG_BUF_SIZE - ret, "revision : %d\n", d->fw.revision);
	ret += snprintf(buffer + ret, LOG_BUF_SIZE - ret, "product id : %s\n", d->fw.product_id);
...
static int ic_exception_check(struct device *dev, char *buf)
{
	...
	if ((lge_get_boot_mode() > LGE_BOOT_MODE_NORMAL) || boot_mode > 0) {
		if (d->fw.revision < 2 || d->fw.revision > 3 || d->fw.version[0] != 1) {
			TOUCH_I("ic_revision : %d, fw_version : v%d.%02d\n",
				d->fw.revision, d->fw.version[0], d->fw.version[1]);
...
static void start_firmware(struct device *dev)
{
	...
	lg4894_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, (void *)&cmd, sizeof(u32));
	lg4894_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data, sizeof(u32));
	lg4894_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data, sizeof(u32));
...
static int stop_firmware(struct device *dev, u32 wdata)
{
	...
	do {
		...
		lg4894_reg_read(dev, ADDR_CMD_REG_SIC_GETTER_READYSTATUS, &read_val, sizeof(u32));
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata)
{
	...
	case CMD_BASE_O_DATA:
		...
		break;
	case CMD_DELTADATA:
		delta_buf = kzalloc(sizeof(u16) * ((COL_SIZE + 2) * (ROW_SIZE + 2)), GFP_KERNEL);
		if (delta_buf == NULL) {
...
static ssize_t show_delta(struct device *dev, char *buf)
{
	...
	if (ret2 == 1) {
		...
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "Test fail (Check if LCD is OFF)\n");
		goto error;
	}
...
static ssize_t show_debug(struct device *dev, char *buf)
{
	...
	for (i = 0; i < DEBUG_ROW_SIZE; i++) {
		...
		for (j = 0; j < DEBUG_COL_SIZE; j++) {
			ret += snprintf(buf + ret, PAGE_SIZE - ret,
					"%5d ", debugdata[i * DEBUG_COL_SIZE + j]);
			log_ret += snprintf(log_buf + log_ret,
					    LOG_BUF_SIZE - log_ret,
					    "%5d ", debugdata[i * DEBUG_COL_SIZE + j]);
		}
