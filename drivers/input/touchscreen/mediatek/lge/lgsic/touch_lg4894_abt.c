...
static int abt_force_set_report_mode(struct device *dev, u32 mode);
static int ReadMemory(struct device *dev, u16 nMemAddr, u16 offset_addr,
...
static int32_t abt_ksocket_init_send_socket(struct sock_comm_t *abt_socket)
{
	...
	ret = sock_create(AF_INET, SOCK_DGRAM, IPPROTO_UDP, &(abt_socket->sock_send));
	...
	ret = sock->ops->connect(sock,
				 (struct sockaddr *)addr, sizeof(struct sockaddr), !O_NONBLOCK);

	if (ret < 0) {
		TOUCH_I(MODULE_NAME ": Could not connect to send socket," "error = %d\n", -ret);
		goto error;
	} else {
		*connected = 1;
...
static void abt_ksocket_start_for_pctool(struct device *dev)
{
	...
	err = sock_create(AF_INET, SOCK_STREAM, IPPROTO_TCP, &abt_comm.ts_sock);
	sock = abt_comm.ts_sock;
	...
	while (1) {
		...
		size = abt_ksocket_receive(&buf[0], sizeof(struct s_comm_packet));

		if (size < 1) {
			TOUCH_E(MODULE_NAME ": RECEIVE sock_recvmsg invalid = %d\n", size);
			break;
		}
...
static void abt_ksocket_start_for_abtstudio(struct device *dev)
{
	...
	if (err < 0) {
		TOUCH_I(": could not create a datagram socket, error = %d\n", -ENXIO);
		goto out;
	}
	...
	err = abt_comm.sock->ops->bind(abt_comm.sock,
				       (struct sockaddr *)&abt_comm.addr, sizeof(struct sockaddr));
	if (err < 0) {
		TOUCH_I(MODULE_NAME ": Could not bind to receive socket, error = %d\n", -err);
		goto out;
	}
	...
	while (1) {
		...
		if (size < 0)
			TOUCH_I(": error getting datagram, sock_recvmsg error = %d\n", size);
...
static uint32_t abt_ksocket_send_exit(void)
{
	...
	if (ret < 0) {
		TOUCH_I(MODULE_NAME ": could not create a datagram socket, error = %d\n", -ENXIO);
		goto error;
	}
...
static int abt_ksocket_send(struct socket *sock,
			    struct sockaddr_in *addr, unsigned char *buf, int len)
{
	struct msghdr msg;
	struct iovec iov;
	mm_segment_t oldfs;
...
static int abt_set_report_mode(struct device *dev, u32 mode)
{
	...
	TOUCH_I("[ABT](%s)mode:%d\n", __func__, mode);
	if (abt_report_mode == mode) {
		TOUCH_I("[ABT](%s) mode(%d) is already set\n", __func__, mode);
		return 0;
	}

	if (mode < 0) {
		TOUCH_I("[ABT](%s) mode(%d) is invalid\n", __func__, mode);
		return -EINVAL;
	}
...
static uint32_t abt_ksocket_rcv_from_pctool(uint8_t *buf, uint32_t len)
{
	...
	if (abt_comm.recv_packet->hdr.cmd == TCP_REG_READ) {
		...
		if (ret < 0)
			TOUCH_I(MODULE_NAME ": TCP REG READ spi_read error : %d\n\n", ret);
		...
		abt_ksocket_send(abt_comm.ts_sock,
				 ...
				 sizeof(TPacketHdr) + abt_comm.recv_packet->data.value);
		...
		if (ret < 0) {
			TOUCH_I(MODULE_NAME ": TCP REG WRITE spi_read error : %d\n\n", ret);
		...
		abt_ksocket_send(abt_comm.ts_sock,
				 &abt_comm.ts_addr,
				 (u8 *) &abt_comm.send_packet, sizeof(TPacketHdr) + sizeof(u32));
	}
...
static uint32_t abt_ksocket_rcv_from_abtstudio(uint8_t *buf, uint32_t len)
{
	...
	case CMD_RAW_DATA_REPORT_MODE_WRITE:
		TOUCH_I(MODULE_NAME ": mode setting - %d\n", val);
		abt_set_report_mode(abt_comm.dev, val);
		break;

	case CMD_RAW_DATA_COMPRESS_WRITE:
		lg4894_write_value(abt_comm.dev, CMD_RAW_DATA_COMPRESS_WRITE, val);
...
static void abt_ksocket_exit(void)
{
	...
	if (abt_comm.thread == NULL) {
		...
		TOUCH_I(MODULE_NAME ": no kernel thread to kill\n");
	...
	if (err < 0) {
		...
		TOUCH_I(MODULE_NAME ": successfully killed kernel thread!\n");
	}
...
int32_t abt_ksocket_raw_data_send(uint8_t *buf, uint32_t len)
{
	...
	if (abt_comm.send_connected == 1) {
		ret = abt_ksocket_send(abt_comm.sock_send, &abt_comm.addr_send, buf, len);
	} else {
		connect_error_count++;
		if (connect_error_count > 10) {
			TOUCH_I(": connection error - socket release\n");
...
static int32_t abt_ksocket_init(struct device *dev,
				char *ip, uint32_t(*listener) (uint8_t *buf, uint32_t len))
{
	...
	if (abt_conn_tool == ABT_STUDIO) {
		abt_comm.thread =
		    kthread_run((void *)abt_ksocket_start_for_abtstudio, dev, MODULE_NAME);
	} else if (abt_conn_tool == TOUCH_SOLUTION) {
		abt_comm.thread =
		    kthread_run((void *)abt_ksocket_start_for_pctool, dev, MODULE_NAME);
	}

	if (IS_ERR(abt_comm.thread)) {
		TOUCH_I(MODULE_NAME ": unable to start kernel thread\n");
		abt_comm.thread = NULL;
		return -ENOMEM;
	}
...
ssize_t store_abtApp(struct device *dev, const char *buf, size_t count)
{
	...
	case REPORT_OFF:
		...
		break;
	default:
		TOUCH_I("[ABT] show mode unknown : %d\n", mode);
		break;
	}
...
ssize_t show_abtTool(struct device *dev, char *buf)
{
	...
	TOUCH_I(MODULE_NAME ":read raw report mode - mode:%d ip:%s\n", buf[0], (char *)&buf[1]);

	return 20;
}

ssize_t store_abtTool(struct device *dev, const char *buf, size_t count)
{
	int mode = buf[0];
	char *ip = (char *)&buf[1];
	bool setFlag = false;

	TOUCH_I(":set raw report mode - mode:%d IP:%s\n", mode, ip);
	...
	case 1:
		...
		if (abt_comm.thread == NULL) {
			...
			abt_ksocket_init(dev, (u8 *) ip, abt_ksocket_rcv_from_abtstudio);
			sic_set_get_data_func(1);
			setFlag = true;
		} else {
			if (memcmp((u8 *) abt_comm.send_ip, (u8 *) ip, 16) != 0) {
				TOUCH_I(":IP change->ksocket exit n init\n\n");
				abt_ksocket_exit();
				abt_ksocket_init(dev, (u8 *) ip, abt_ksocket_rcv_from_abtstudio);
				setFlag = true;
			}
	...
	case 2:
		...
		if (abt_comm.thread == NULL) {
			TOUCH_I(": mode Touch Solution Start\n\n");
			abt_ksocket_init(dev, (u8 *) ip, abt_ksocket_rcv_from_pctool);
			...
			TOUCH_I(": abt_comm.thread Not NULL\n\n");
			if (memcmp((u8 *) abt_comm.send_ip, (u8 *) ip, 16) != 0) {
				abt_ksocket_exit();
				abt_ksocket_init(dev, (u8 *) ip, abt_ksocket_rcv_from_pctool);
			} else {
...
int abt_force_set_report_mode(struct device *dev, u32 mode)
{
	...
	/* send debug mode */
	ret = lg4894_write_value(dev, CMD_RAW_DATA_REPORT_MODE_WRITE, wdata);
	abt_report_mode = mode;

	/* receive debug report buffer */
	if (mode >= 0) {
		ret = lg4894_read_value(dev, CMD_RAW_DATA_REPORT_MODE_READ, &rdata);
		TOUCH_I("(%d)rdata\n", rdata);
		if (ret < 0 || rdata <= 0) {
...
error:
	wdata = 0;
	lg4894_write_value(dev, CMD_RAW_DATA_REPORT_MODE_WRITE, wdata);
	abt_report_mode = 0;
...
void lg4894_sic_abt_init(struct device *dev)
{
	...
	memcpy((u8 *) &abt_head[0], (u8 *) &tHeadBuffer, sizeof(struct T_ABTLog_FileHead));
}
...
int lg4894_sic_abt_irq_handler(struct device *dev)
{
	...
	if (lg4894_reg_read(dev, tc_ic_status, &all_data[0], sizeof(all_data)) < 0) {
...
void lg4894_sic_abt_onchip_debug(struct device *dev, u8 *all_data)
{
	...
	if (abt_ocd_on) {
		...
		if (u8_ocd_pieces_cnt == 0 && u8_ocd_pieces_cnt != prev_rnd_piece_no)
			abt_ocd_read ^= 1;
			...
			if (u8_ocd_pieces_cnt != DEF_RNDCPY_EVERY_Nth_FRAME - 1) {
				...
				if ((i * 2) % MAX_RW_SIZE != 0) {
					...
					if (ret < 0)
						TOUCH_E("RNdata reg addr write fail [%d]\n",
							u8_ocd_pieces_cnt);
					_u8_data_ptr += (sizeof(u8) * ((i * 2) % MAX_RW_SIZE));
					u32_dbg_offset += (sizeof(u8) *
							   ((i * 2) % MAX_RW_SIZE) / 4);
				}
				memcpy(abt_reportP, all_data, sizeof(u8) * 34 * 4);
				memcpy(&abt_reportP[34 * 4], &all_data[34 * 4], sizeof(u8) * 112);
	       ...
	       u16 nOffsetValueByteSize, u8 *pData, u32 nSize)
{
	...
	while (nRead > 0) {
		lg4894_reg_write(dev, offset_addr, (u8 *) &nOffset, nOffsetValueByteSize);
...
void lg4894_sic_abt_report_mode(struct device *dev, u8 *all_data)
{
	...
	if (abt_report_mode) {
		...
		if (d_header->type == abt_report_mode) {
			...
			if (d_header->data_size % MAX_RW_SIZE != 0) {
				...
				d_data_ptr += (sizeof(u8) * (d_header->data_size % MAX_RW_SIZE));
	...
	if ((u8 *) d_data_ptr - (u8 *) packet_ptr > 0) {
		...
		packet_ptr->timestamp = t_stamp.tv_sec * 1000000 + t_stamp.tv_usec;
		...
		abt_ksocket_raw_data_send((u8 *) packet_ptr, (u8 *) d_data_ptr - (u8 *) packet_ptr);
	}
}
