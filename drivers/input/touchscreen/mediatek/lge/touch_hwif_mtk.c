...
int touch_bus_init(struct device *dev, int buf_size)
{
	...
	if (buf_size) {
		ts->tx_buf = (u8 *) dma_alloc_coherent(NULL,
						       buf_size, (dma_addr_t *) (long)&ts->tx_pa,
						       GFP_KERNEL);
		...
		ts->rx_buf = (u8 *) dma_alloc_coherent(NULL,
						       buf_size, (dma_addr_t *) (long)&ts->rx_pa,
						       GFP_KERNEL);
		if (!ts->rx_buf)
	...
	if (IS_ERR_OR_NULL(ts->pinctrl.ctrl)) {
		...
		ts->pinctrl.active = pinctrl_lookup_state(ts->pinctrl.ctrl, "touch_pin_active");

		if (IS_ERR_OR_NULL(ts->pinctrl.active))
			TOUCH_E("cannot get pinctrl.active\n");

		ts->pinctrl.suspend = pinctrl_lookup_state(ts->pinctrl.ctrl, "touch_pin_sleep");

		if (IS_ERR_OR_NULL(ts->pinctrl.suspend))
			TOUCH_E("cannot get pinctrl.suspend\n");

		if (!IS_ERR_OR_NULL(ts->pinctrl.active)) {
			ret = pinctrl_select_state(ts->pinctrl.ctrl, ts->pinctrl.active);
			if (ret)
...
int touch_boot_mode_check(struct device *dev)
{
	...
	/* ret = lge_get_factory_boot(); [daehyun temp] */
	ret = TOUCH_NORMAL_BOOT;	/* [daehyun temp] */
