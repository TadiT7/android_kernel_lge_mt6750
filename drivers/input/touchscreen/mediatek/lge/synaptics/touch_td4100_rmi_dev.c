...
static ssize_t rmidev_sysfs_pid_show(struct device *dev, struct device_attribute *attr, char *buf);
			   ...
			   unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	/* page restore */
	if (page_changed) {
		ret = synaptics_write(dev, PAGE_SELECT_REG, &page_old, sizeof(page_old));
...
static irqreturn_t rmidev_sysfs_irq(int irq, void *dev_id)
{
	...
	sysfs_notify(&ts->input->dev.kobj, SYSFS_FOLDER_NAME, "attn_state");
...
static int rmidev_sysfs_irq_enable(struct device *ts_dev, bool enable)
{
	...
	unsigned long irq_flags = IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT;
	...
	if (enable) {
		...
		retval = rmidev_i2c_read(ts_dev, d->f01.dsc.data_base + 1, &intr_status, 1);
		if (retval < 0)
			return retval;
...
/*	unsigned char intr_status = 0;	*/
	...
	if (length) {
		retval = rmidev_i2c_read(rmidev->ts_dev, address, (unsigned char *)buf, length);
		if (retval < 0) {
				       ...
				       char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = rmidev_i2c_write(rmidev->ts_dev, address, (unsigned char *)buf, length);
		if (retval < 0) {
...
/*	struct synaptics_rmi4_fn *fhandler;	*/
	...
	retval = rmidev_i2c_read(rmidev->ts_dev, *f_pos, rmidev->tmpbuf, count);
	if (retval < 0)
		goto clean_up;
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	retval = rmidev_i2c_write(rmidev->ts_dev, *f_pos, rmidev->tmpbuf, count);
	if (retval >= 0)
		*f_pos += retval;
...
static void rmidev_attn(struct device *ts_dev, unsigned char intr_mask)
{
	TOUCH_TRACE();
...
static int rmidev_init_device(struct device *ts_dev)
{
	...
	if (retval < 0) {
		...
		retval = gpio_export_link(&(ts->input->dev), "attn", ts->int_pin);
		if (retval < 0)
	...
	rmidev->sysfs_dir = kobject_create_and_add(SYSFS_FOLDER_NAME, &ts->input->dev.kobj);
	if (!rmidev->sysfs_dir) {
...
{
	TOUCH_TRACE();
	TOUCH_I("%s: rmidev disable for bring up(temp)\n", __func__);
	return 0;
