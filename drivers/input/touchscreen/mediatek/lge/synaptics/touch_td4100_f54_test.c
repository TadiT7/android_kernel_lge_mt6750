...
#include "touch_td4100_f54_test.h"
#include "touch_td4100_test_limits.h"
...
int CompareNoiseReport(void)
{
	...
	for (i = 0; i < (int)RxChannelCount; i++)
		wlen += snprintf(wbuf + wlen, sizeof(wbuf) - wlen, " [%2d] ", i);

	for (i = 0; i < TxChannelCount; i++) {
		wlen += snprintf(wbuf + wlen, sizeof(wbuf) - wlen, "\n[%2d] ", i);
		for (j = 0; j < RxChannelCount; j++) {
			ImagepF[i][j] = NoiseDeltaMax[i][j] - NoiseDeltaMin[i][j];
			wlen += snprintf(wbuf + wlen, sizeof(wbuf) - wlen, "%5d ", ImagepF[i][j]);
		}
	}
	wlen += snprintf(wbuf + wlen, sizeof(wbuf) - wlen, "\n");
...
int ReadImageReport(struct device *dev)
{
	...
	Read8BitRegisters(dev, (F54DataBase + REPORT_DATA_OFFEST), &Data[0], MaxArrayLength);
	...
	for (i = 0; i < (int)TxChannelCount; i++) {
		...
		for (j = 0; j < (int)RxChannelCount; j++) {
			...
			f54len += snprintf(f54buf + f54len,
					   sizeof(f54buf) - f54len, "%5d ", ImagepF[i][j]);
			k = k + 2;
	...
	*buf = 0;
	...
	for (i = 0; i < (int)RxChannelCount; i++) {
		...
		for (j = 0; j < (int)TxChannelCount; j++)
			ret += snprintf(buf + ret, PAGE_SIZE - ret, "%5d ", ImagepF[j][i]);
...
int ReadHighResistanceReport(struct device *dev)
{
	...
	f54len += snprintf(f54buf + f54len, sizeof(f54buf) - f54len, "[High Resistance Test]\n");
	...
	f54len += snprintf(f54buf + f54len, sizeof(f54buf) - f54len, "Min(pF) = %d\n", minpF);
...
int ReadAbsRawOpen(struct device *dev)
{
	...
	for (i = 0; i < ((int)F12_2DRxCount + (int)F12_2DTxCount); i++) {
		...
		if (i < (int)F12_2DRxCount) {
			...
			f54len += snprintf(f54buf + f54len,
					   sizeof(f54buf) - f54len, "%5d ", AbsOpen[i]);
		} else {
			TOUCH_I("Tx[%d]: %d, ", i - (int)F12_2DRxCount, AbsOpen[i]);
			f54len += snprintf(f54buf + f54len,
					   sizeof(f54buf) - f54len, "%5d ", AbsOpen[i]);
		}
...
int ReadAbsRawShort(struct device *dev)
{
	...
	for (i = 0; i < ((int)F12_2DRxCount + (int)F12_2DTxCount); i++) {
		...
		if (i < (int)F12_2DRxCount) {
			...
			f54len += snprintf(f54buf + f54len,
					   sizeof(f54buf) - f54len, "%5d ", AbsShort[i]);
		} else {
			TOUCH_I("Tx[%d]: %d, ", i - (int)F12_2DRxCount, AbsShort[i]);
			f54len += snprintf(f54buf + f54len,
					   sizeof(f54buf) - f54len, "%5d ", AbsShort[i]);
		}
...
int ReadSensorSpeedReport(struct device *dev)
{
	...
	wlen += snprintf(wbuf + wlen, sizeof(wbuf) - wlen, "[Sensor Speed Test]\n");
...
void ReadTRexOpenReport(struct device *dev)
{
	...
	Read8BitRegisters(dev, (F54DataBase + REPORT_DATA_OFFEST), &Data[0], TRX_BITMAP_LENGTH);
...
int ReadTRexGroundReport(struct device *dev)
{
	...
	f54len += snprintf(f54buf + f54len, sizeof(f54buf) - f54len, "[TRex Gnd Test]\n");
...
int ReadTRexShortReport(struct device *dev)
{
	...
	f54len += snprintf(f54buf + f54len, sizeof(f54buf) - f54len, "[TRex Short Test]\n");
	...
	do {
		...
		case 0x12:
			if (!bHaveF12) {
				...
				Read8BitRegisters(dev, (F12ControlBase + offset), Data, 14);
		...
		case 0x54:
			if (!bHaveF54) {
				...
				if (bHaveSlewMetric || bHaveHBlank
				    || bHaveVBlank
				    || bHaveLongHBlank || bHaveNoiseMitigation2 || bHaveSlewOption)
					offset += 1;	/* Ctrl 63 */
				...
				if (bHaveF54Query25)
					q_offset++;	/* Query 25 */
				bHaveF54Ctrl106 = ((Data[q_offset] & 0x01) == 0x01);
				bHaveF54Ctrl107 = ((Data[q_offset] & 0x04) == 0x04);
				bHaveF54Ctrl108 = ((Data[q_offset] & 0x08) == 0x08);
				bHaveF54Ctrl109 = ((Data[q_offset] & 0x10) == 0x10);
				bHaveF54Query27 = ((Data[q_offset] & 0x80) == 0x80);
...
/* The following function illustrates
...
int AbsRaw(struct device *dev, int mode, char *buf)
{
	...
	if (bHaveCtrl98) {
		Read8BitRegisters(dev, (F54ControlBase + F54Ctrl98Offset), &Data[0], 6);
