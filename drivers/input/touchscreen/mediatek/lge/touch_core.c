...
/* #include "./../../../misc/mediatek/video/mt6755/videox/disp_recovery.h" [daehyun temp] */
	...
	for (i = 0; i < MAX_FINGER; i++) {
		if (old_mask & (1 << i)) {
			...
			TOUCH_I("finger canceled:<%d>(%4d,%4d,%4d)\n",
				i, ts->tdata[i].x, ts->tdata[i].y, ts->tdata[i].pressure);
		}
	}
...
static void touch_report_event(struct touch_core_data *ts)
{
	...
	for (i = 0; i < MAX_FINGER; i++) {
		if (new_mask & (1 << i)) {
			input_mt_slot(ts->input, i);
			input_report_abs(ts->input, ABS_MT_TRACKING_ID, ts->tdata[i].id);
			input_report_abs(ts->input, ABS_MT_POSITION_X, ts->tdata[i].x);
			input_report_abs(ts->input, ABS_MT_POSITION_Y, ts->tdata[i].y);
			input_report_abs(ts->input, ABS_MT_PRESSURE, ts->tdata[i].pressure);
			input_report_abs(ts->input, ABS_MT_WIDTH_MAJOR, ts->tdata[i].width_major);
			input_report_abs(ts->input, ABS_MT_WIDTH_MINOR, ts->tdata[i].width_minor);
			input_report_abs(ts->input, ABS_MT_ORIENTATION, ts->tdata[i].orientation);
			...
			if (press_mask & (1 << i)) {
				if (hide_coordinate && (lockscreen == LOCKSCREEN_LOCK)) {
					...
					TOUCH_I("%d finger pressed:<%d>(%4d,%4d,%4d)\n",
						...
						i,
						ts->tdata[i].x,
						ts->tdata[i].y, ts->tdata[i].pressure);
				}
			}
...
irqreturn_t touch_irq_thread(int irq, void *dev_id)
{
	...
	if (ret >= 0) {
		...
		if (ret == -ERESTART) {
#if defined(CONFIG_LGE_TOUCH_LG4894) || defined(CONFIG_LGE_TOUCH_MELFAS_MIT300)
...
{
	struct touch_core_data *ts = container_of(to_delayed_work(init_work),
...
{
	struct touch_core_data *ts = container_of(to_delayed_work(upgrade_work),
	...
	mod_delayed_work(ts->wq, &ts->init_work, msecs_to_jiffies(ts->caps.hw_reset_delay));
}

static void touch_fb_work_func(struct work_struct *fb_work)
{
	struct touch_core_data *ts = container_of(to_delayed_work(fb_work),
...
static int touch_check_driver_function(struct touch_core_data *ts)
{
	if (ts->driver->probe &&
	    ...
	    ts->driver->suspend &&
	    ts->driver->resume &&
	    ts->driver->init && ts->driver->irq_handler && ts->driver->power && ts->driver->upgrade)
		return 0;
...
static int touch_init_input(struct touch_core_data *ts)
{
	...
	TOUCH_I("%s %d-%d-%d-%d-%d-%d-%d\n", __func__,
		...
		ts->caps.max_width, ts->caps.max_width, ts->caps.max_orientation, ts->caps.max_id);
	...
	input_set_abs_params(input, ABS_MT_POSITION_X, 0, ts->caps.max_x, 0, 0);
	input_set_abs_params(input, ABS_MT_POSITION_Y, 0, ts->caps.max_y, 0, 0);
	input_set_abs_params(input, ABS_MT_PRESSURE, 0, ts->caps.max_pressure, 0, 0);
	input_set_abs_params(input, ABS_MT_WIDTH_MAJOR, 0, ts->caps.max_width, 0, 0);
	input_set_abs_params(input, ABS_MT_WIDTH_MINOR, 0, ts->caps.max_width, 0, 0);
	input_set_abs_params(input, ABS_MT_ORIENTATION, 0, ts->caps.max_orientation, 0, 0);
...
{
	struct touch_core_data *ts = container_of(h, struct touch_core_data, early_suspend);
...
static int touch_notify(struct touch_core_data *ts, unsigned long event, void *data)
{
	int ret = 0;
...
{
	struct touch_core_data *ts = container_of(this, struct touch_core_data, blocking_notif);
...
{
	struct touch_core_data *ts = container_of(to_delayed_work(notify_work),
...
static int touch_atomic_notifier_callback(struct notifier_block *this,
					  unsigned long event, void *data)
{
	struct touch_core_data *ts = container_of(this, struct touch_core_data, atomic_notif);
...
static int touch_core_probe_normal(struct platform_device *pdev)
{
	...
	ret = touch_request_irq(ts->irq, touch_irq_handler,
				touch_irq_thread, ts->irqflags | IRQF_ONESHOT, LGE_TOUCH_NAME, ts);
	if (ret) {
