...
static int synaptics_tci_knock(struct device *dev)
{
	...
	for (i = 0; i < 2; i++) {
		if ((ts->tci.mode & (1 << i)) == 0x0) {
			...
			TOUCH_I("[TCI %d]: tap_count(%4d), intertap_time(%4d/%4d),",
				i + 1, info->tap_count, info->min_intertap, info->max_intertap);
			TOUCH_I("touch_slop(%4d), tap_distance(%4d), intr_delay(%4d)\n",
...
static ssize_t store_reg_ctrl(struct device *dev, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%s %d %x %d %x ", command, &page, &reg, &offset, &value) <= 0)
		return count;
