...
static int RspReadImageReport(struct device *dev, int16_t *buf)
{
	...
	ret = synaptics_read(dev, REPORT_DATA_REG, (u8 *) buf, REPORT_DATA_LEN);
	if (ret < 0)
		return ret;
...
static int TD4100_Get_Report_Data(struct device *dev, int report_type, int16_t *result_buf,
				  int buf_size)
{
	struct synaptics_data *d = to_synaptics_data(dev);
	int ret = 0;
...
static int TD4100_Get_AMP_Short_Data(struct device *dev)
{
	...
	/* F54_ANALOG_CTRL103 (TDDI CBC Enable) */
	ret = synaptics_read(dev, TDDI_CBC_ENABLE_CTRL_REG, get_left_cbc, sizeof(get_left_cbc));
	/* F54_ANALOG_CTRL103 (TDDI CBC Enable) */
	ret |= synaptics_read(dev, TDDI_CBC_ENABLE_CTRL_REG, get_right_cbc, sizeof(get_right_cbc));
	if (ret < 0) {
	...
	/* F54_ANALOG_CTRL103 (TDDI CBC Enable) */
	ret |= synaptics_write(dev, TDDI_CBC_ENABLE_CTRL_REG, get_left_cbc, sizeof(get_left_cbc));
	if (ret < 0) {
	...
	/* F54_ANALOG_CTRL103 (TDDI CBC Enable) */
	ret |= synaptics_write(dev, TDDI_CBC_ENABLE_CTRL_REG, get_right_cbc, sizeof(get_right_cbc));
	if (ret < 0) {
	...
	/* Calculate(Divided) the delta value between A and B */
	for (i = 0; i < ROW_SIZE; i++) {
		for (j = 0; j < COL_SIZE; j++) {
...
static int TD4100_Get_AMP_Open_Data(struct device *dev)
{
	...
	/* F54_ANALOG_CTRL99 (Integration Duration) */
	ret =
	    synaptics_read(dev, WAVEFORM_DURATION_CTRL_REG, original_intdur,
			   sizeof(original_intdur));
	if (ret < 0) {
	...
	/* F54_ANALOG_CTRL99 (Integration Duration) */
	ret = synaptics_read(dev, WAVEFORM_DURATION_CTRL_REG, intdur_phase1, sizeof(intdur_phase1));
	if (ret < 0) {
	...
	/* F54_ANALOG_CTRL99 (Integration Duration) */
	ret =
	    synaptics_write(dev, WAVEFORM_DURATION_CTRL_REG, intdur_phase1, sizeof(intdur_phase1));
	if (ret < 0) {
	...
	/* F54_ANALOG_CTRL99 (Integration Duration) */
	ret |=
	    synaptics_read(dev, WAVEFORM_DURATION_CTRL_REG, intdur_phase2, sizeof(intdur_phase2));
	if (ret < 0) {
	...
	/* F54_ANALOG_CTRL99 (Integration Duration) */
	ret =
	    synaptics_write(dev, WAVEFORM_DURATION_CTRL_REG, intdur_phase2, sizeof(intdur_phase2));
	if (ret < 0) {
	...
	/* F54_ANALOG_CTRL99 (Integration Duration) */
	ret =
	    synaptics_write(dev, WAVEFORM_DURATION_CTRL_REG, original_intdur,
			    sizeof(original_intdur));
	if (ret < 0) {
...
static int TD4100_Test_Result_Data(struct device *dev, int test_type, int *test_result)
{
	...
	*test_result = TEST_FAIL;
	...
	case RAW_DATA_TEST:
		...
		ret +=
		    snprintf(buffer + ret, PAGE_SIZE - ret,
			     "============= Raw Data Test Result =============\n");
	...
	case P2P_NOISE_TEST:
		...
		ret +=
		    snprintf(buffer + ret, PAGE_SIZE - ret,
			     "===== Peak-to-Peak Noise Test Result =====\n");
	...
	for (i = ROW_SIZE - 1; i >= 0; i--) {
		...
		for (j = 0; j < COL_SIZE; j++) {
			...
			if (check_limit_upper && result_data[i][j] > limit_upper) {
				ret +=
				    snprintf(buffer + ret, PAGE_SIZE - ret, "!%4d ",
					     result_data[i][j]);
				fail_count++;
				...
				ret +=
				    snprintf(buffer + ret, PAGE_SIZE - ret, "%5d ",
					     result_data[i][j]);
			}
	...
	if (fail_count && (check_limit_upper || check_limit_lower)) {
		ret +=
		    snprintf(buffer + ret, PAGE_SIZE - ret, "Test FAIL : %d Errors\n", fail_count);
	} else {
...
static int synaptics_sd(struct device *dev, char *buf, int *raw, int *ch)
{
	...
	TOUCH_I("raw_result : %d / jitter_result : %d / short_result : %d / open_result : %d\n",
		raw_result, jitter_result, short_result, open_result);
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	if (ts->lpwg.screen == 0) {
		...
		ret_size =
		    snprintf(buffer, PAGE_SIZE, "LCD Off state. please turn on the display\n");
	...
	if (chstatus != TEST_PASS) {	/* FAIL */
		ret_size +=
		    snprintf(buffer + ret_size, PAGE_SIZE - ret_size, "Channel Status : Fail\n");
		TOUCH_I("Channel Status : Fail\n");
	} else {		/* PASS */
		ret_size +=
		    snprintf(buffer + ret_size, PAGE_SIZE - ret_size, "Channel Status : Pass\n");
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	if (lpwg_rawstatus != TEST_PASS) {	/* FAIL */
		ret_size +=
		    snprintf(buffer + ret_size, PAGE_SIZE - ret_size, "LPWG RawData : Fail\n");
		TOUCH_I("LPWG RawData : Fail\n");
	} else {		/* PASS */
		ret_size +=
		    snprintf(buffer + ret_size, PAGE_SIZE - ret_size, "LPWG RawData : Pass\n");
