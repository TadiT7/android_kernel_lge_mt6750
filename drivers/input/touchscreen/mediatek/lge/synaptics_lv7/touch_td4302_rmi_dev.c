...
static int rmidev_sysfs_irq_enable(struct device *ts_dev, bool enable)
{
	...
	if (enable) {
		...
		retval = synaptics_read(ts_dev,
					...
					d->f01.dsc.data_base + 1, &intr_status, 1);
		if (retval < 0)
			return retval;
...
/*	unsigned char intr_status = 0;	*/
	...
	if (length) {
		retval = synaptics_read(rmidev->ts_dev,
					address >> 8, address, (unsigned char *)buf, length);
		if (retval < 0) {
...
/*	struct synaptics_rmi4_fn *fhandler;	*/
	...
	retval = synaptics_read(rmidev->ts_dev, (*f_pos) >> 8, *f_pos, rmidev->tmpbuf, count);
	if (retval < 0)
		goto clean_up;
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	retval = synaptics_write(rmidev->ts_dev, (*f_pos) >> 8, *f_pos, rmidev->tmpbuf, count);
	if (retval >= 0)
		*f_pos += retval;
