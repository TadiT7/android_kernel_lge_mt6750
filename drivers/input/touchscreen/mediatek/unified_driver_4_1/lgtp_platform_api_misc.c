...
static int TOUCHIrqRegistration(TouchDriverData *pDriverData, irq_handler_t irqHandler,
				irq_handler_t threaded_irqHandler)
{
		     ...
		     irq_handler_t threaded_irqHandler)
{
...
#if defined(TOUCH_PLATFORM_QCT)
	{
		...
		ret =
		    request_irq(pDriverData->client->irq, irqHandler, TOUCH_IRQ_FLAGS,
