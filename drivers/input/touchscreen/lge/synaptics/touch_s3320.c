...
#ifdef TARGET_MT6732_C90
        lpwg_data[0] = ((info->tap_count << 3) & 0xf8) | (lpwg_data[0] & 0x7);
#else
        lpwg_data[0] |= ((info->tap_count << 3) | 1);
#endif
	lpwg_data[1] = info->min_intertap;
...
static int synaptics_get_panel_id(struct device *dev)
{
	...
	if (panel_id_init && d->fw.panel_id) {
		TOUCH_I("panel id : %d", d->fw.panel_id);
	...
	TOUCH_I("Touch panel id : %d", d->fw.panel_id);
...
#ifdef TARGET_MT6732_C90
	d->fw.fw_pid_addr = 0x16d04;
	d->fw.fw_ver_addr = 0x16d00;
#else
