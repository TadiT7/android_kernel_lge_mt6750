...
//	u8 rdata = 0;
	...
	for (i = 0; i < 2; i++) {
		...
		ret |= ft5526_reg_read(dev, FTS_REG_LIB_VER_H, (u8 *)&lib_version_high, 1);
		ret |= ft5526_reg_read(dev, FTS_REG_LIB_VER_L, (u8 *)&lib_version_low, 1);
...
//	ft5526_lpwg_mode(dev, 1);
	...
	if(apk_allocate_flag == 1) {
		...
		if (!tpd_i2c_dma_va)
			...
			TOUCH_E("dma_alloc_coherent success!\n");
...
static int ft5526_get_cmd_version(struct device *dev, char *buf)
{
	...
	if (d->ic_info.chip_id == 0x54) {
		offset += snprintf(buf + offset, PAGE_SIZE - offset, "Product-id : [FT5526]\n\n");
	}
	else {
