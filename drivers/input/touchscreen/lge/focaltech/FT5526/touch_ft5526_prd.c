...
#define NOISE_MIN 0
#define EXTRA_JITTER_MAX 210
#define EXTRA_JITTER_MIN 0
...
static s16 pre_rawdata_buf[MAX_ROW][MAX_COL];
...
#ifdef CONFIG_EXTRA_NOISE_TEST /* extra jitter test */
static s16 extra_jitter_buf[MAX_ROW][MAX_COL];
static int rawdata_max[MAX_ROW][MAX_COL];
static int rawdata_sum_data[MAX_ROW][MAX_COL];
#endif
...
#ifdef CONFIG_EXTRA_NOISE_TEST
int ft5526_prd_get_extra_noise_data(struct device *dev)
{
	int i, j = 0;
	for (i = 0; i < MAX_ROW; i++) {
		for(j = 0; j < MAX_COL; j++) {
			fts_data[i][j] = extra_jitter_buf[i][j];
		}
	}
	return 0;
}
...
int ft5526_prd_get_noise_data(struct device *dev)
{
	...
	for (k = 0; k < frame_num; k++) /* read rawdata repeatedly during frame_num's frame*/
	{
...
//			TOUCH_I("Read Raw data at once\n");
			...
			ret = ft5526_reg_read(dev, 0x36, &i2c_data[0], MAX_COL*MAX_ROW*2);    // TH8 Column:21 Row:34
			if(ret < 0) {
				TOUCH_E("i2c error\n");
				return ret;
			}
			...
			if (k !=0) {
				for (i = 0; i < MAX_ROW; i++) {
					for(j = 0; j < MAX_COL; j++) {
						current_rawdata_buf = abs((i2c_data[m] << 8) + i2c_data[m+1]); // current_rawdata;
						rawdata_diff = pre_rawdata_buf[i][j] - current_rawdata_buf;

						if (rawdata_diff < 0)
							rawdata_diff *= -1;

						if (fts_data[i][j] < rawdata_diff ) {
							fts_data[i][j] = rawdata_diff;
						}

						pre_rawdata_buf[i][j] = current_rawdata_buf;
						m+=2;

#ifdef CONFIG_EXTRA_NOISE_TEST
						/* extra jitter data */
						rawdata_sum_data[i][j] += current_rawdata_buf;
						if ( rawdata_max[i][j] < current_rawdata_buf)
							rawdata_max[i][j] = current_rawdata_buf;
#endif
	...
	memset(extra_jitter_buf, 0, sizeof(extra_jitter_buf));

	for (i = 0; i < MAX_ROW; i++) {
		for(j = 0; j < MAX_COL; j++) {
		    rawdata_mean = rawdata_sum_data[i][j]/frame_num;
			extra_jitter_buf[i][j] = rawdata_max[i][j] - rawdata_mean;
		}
	}
#endif
...
//	u8 frame_count;
	...
	ret = ft5526_reg_read(dev, 0x6A, &i2c_data[0], MAX_COL*MAX_ROW*2);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
// Master chip end---------------------------------------------------------------------------------
// Slave chip I2C address W 0x72, R 0x71
// Slave chip read---------------------------------------------------------------------------------
	// Get IB data
	for (i = 0; (total - TEST_PACKET_LENGTH*i) > 0; i++)
	{
...
int ft5526_prd_get_adc_data(struct device *dev)
{
	...
	ret = ft5526_reg_read(dev, 0xB1, &data, 1);  // IC_TYPE READ
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	*test_result = TEST_FAIL;
	...
	switch (test_type) {
		...
		case NOISE_TEST:
			...
			break;
		case EXTRA_JITTER_TEST:
			ret += snprintf(log_buf + ret, LOG_BUF_SIZE - ret, "============= EXTRA Noise Test Result =============\n");
			limit_upper = EXTRA_JITTER_MAX;
			limit_lower = EXTRA_JITTER_MIN;
			break;
		case DELTA_SHOW:
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	if(rawdata_test_result == TEST_FAIL) {
		goto FAIL;
	}
	...
	ret_size = ft5526_prd_test_data(dev, NOISE_TEST, &channel_status_test_result);

	TOUCH_I("Noise Test Result : %d\n", channel_status_test_result);

	write_file(dev, log_buf, TIME_INFO_SKIP);

	if(channel_status_test_result == TEST_FAIL) {
		goto FAIL;
	}
	...
	/* extra noise test */
	ret = ft5526_prd_get_extra_noise_data(dev);

	ret_size = ft5526_prd_test_data(dev, EXTRA_JITTER_TEST, &channel_status_test_result);

	TOUCH_I("Extra Noise Test Result : %d\n", channel_status_test_result);
	...
	ret_size = ft5526_prd_test_data_adc(dev, &channel_status_test_result);
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	ret_size = ft5526_prd_test_data(dev, LPWG_RAW_DATA_TEST, &test_result);
	...
	if (d->ic_info.chip_id_low == 0x07) {
		...
		ret_size = ft5526_prd_test_data(dev, LPWG_NOISE_TEST, &test_result);
...
static ssize_t show_reset_test(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	u8 data = 0;
	...
	ret = ft5526_reg_read(dev, 0x00, &data, 1);
	TOUCH_I("%s : HW Reset ready : %d\n", __func__, data);

	data = 1;
	ret = ft5526_reg_write(dev, 0x00, &data, 1);

	ret = ft5526_reg_read(dev, 0x00, &data, 1);
	TOUCH_I("%s : HW Reset pre : %d\n", __func__, data);
	...
	ret = ft5526_reg_read(dev, 0x00, &data, 1);
	TOUCH_I("%s : HW Reset after : %d\n", __func__, data);

	return ret;
}

static ssize_t show_sw_reset_test(struct device *dev, char *buf)
{

	int ret = 0;
	u8 data = 0;

	ft5526_reg_write(dev, 0x07, &data, 0);

	return ret;
}

static ssize_t show_gpio_low(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static ssize_t show_gpio_high(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static ssize_t show_power_enable(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static ssize_t show_power_disable(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static TOUCH_ATTR(reset_test, show_reset_test, NULL);
static TOUCH_ATTR(gpio_low, show_gpio_low, NULL);
static TOUCH_ATTR(gpio_high, show_gpio_high, NULL);
static TOUCH_ATTR(power_able, show_power_enable, NULL);
static TOUCH_ATTR(power_dis, show_power_disable, NULL);
static TOUCH_ATTR(sw_reset, show_sw_reset_test, NULL);
...
//	&touch_attr_rawdata.attr,
	...
	&touch_attr_reset_test.attr,
	&touch_attr_gpio_low.attr,
	&touch_attr_gpio_high.attr,
	&touch_attr_power_able.attr,
	&touch_attr_power_dis.attr,
	&touch_attr_sw_reset.attr,
	NULL,
};
