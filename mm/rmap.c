					...
					struct vm_area_struct *target_vma)
{
	...
	if (target_vma) {
		...
		return rwc->rmap_one(page, target_vma, address, rwc->arg);
	}
				...
				struct vm_area_struct *target_vma)
{
	...
	if (target_vma) {
		...
		if (unlikely(!list_empty(&mapping->i_mmap_nonlinear)))
			goto done;
		address = vma_address(page, target_vma);
		ret = rwc->rmap_one(page, target_vma, address, rwc->arg);
		goto done;
	} else {
...
{
	if (unlikely(PageKsm(page)))
		return rmap_walk_ksm(page, rwc, vma);
	else if (PageAnon(page))
		return rmap_walk_anon(page, rwc, vma);
	else
		return rmap_walk_file(page, rwc, vma);
}
